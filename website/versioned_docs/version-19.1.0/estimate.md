...
:::note
Our estimation of `fee`, `gasLimit` and `storageLimit` is based on the response of rpc call to `simulate_operation` plus a small buffer, the context and volume at the time of `simulate_operation` might differ from the context and volume at the time of `injection/operation` which might caused errors like `fees_too_low`, `storage_exhausted.operation` and `gas_exhausted.operation`
:::
