...
>[!NOTE]
> This page is now deprecated due to no functioning objkt shadownet smart contract for now.
> We still reference now deprecated ghostnet and it's smart contracts until there is objkt shadownet support
...
<details>
  ...
  ```bash
    ...
    octez-client -E https://shadownet.tezos.ecadinfra.com gen keys mysamplekey -s secp256k1
    octez-client -E https://shadownet.tezos.ecadinfra.com show address mysamplekey -S
  ```
...
const main = async () => {
    ...
    const contract = await Tezos.contract.at('KT1VMN1uDrhuddff6RGndrcJESEjTJGrbw9a');
...
In a new terminal in your projects directory, install Taquito and Taquito's Beacon Wallet package.
```bash
...
const App = () => {
  const [Tezos] = useState<TezosToolkit>(
    new TezosToolkit("https://shadownet.tezos.ecadinfra.com")
  );
