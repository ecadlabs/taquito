...
- [BeaconWallet](wallets) (Built on the DAppClient from the Beacon SDK which handles all the logic related to connecting to beacon-compatible wallets and sending requests. The BeaconWallet provides developers a way to connect a dapp built with Taquito to a wallet giving the freedom to the users of the dapp to choose the wallet they want. Common Beacon types like `NetworkType` can be imported from `@taquito/beacon-wallet/types`, and `BeaconEvent` from `@taquito/beacon-wallet`.)
