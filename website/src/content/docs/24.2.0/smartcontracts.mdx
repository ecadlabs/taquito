...
```js live
try {
  const contract = await Tezos.contract.at('KT1VNTTHJ56xDoBoS1n3HDcr4s2zt6yDshbG');
...
```js live wallet
try {
  const contract = await Tezos.wallet.at('KT1VNTTHJ56xDoBoS1n3HDcr4s2zt6yDshbG');
  const methods = contract.parameterSchema.ExtractSignatures();
  console.log(JSON.stringify(methods, null, 2));
} catch (error) {
  console.log(`Error: ${error}`);
...
```js live
try {
  const contract = await Tezos.contract.at('KT1XbU7axZzQ3pL8mJwEi5HAkW99YhXjaNQq');
...
try {
  ...
  const hash = await operation.confirmation();
