...
The contracts used in the Taquito integration tests and in the Taquito documentation live code examples are test data and require curation. Here we collect the contracts, give them names, demonstrate their properties and describe their use.
...
- **Wallet Contracts**
  ...
  - [WalletAreYouThereContract](#walletareyoutherecontract)

# Basic Contracts

## IncrementContract

This contract serves as the default introductory example. The contract is used to demo addition and subtraction by a smart contract.
...
The contract supports a [Michelson Tutorial](michelsonmap). It has a default endpoint that takes a pair of an address and an amount of tez.

- [See the full tutorial](https://medium.com/coinmonks/working-with-michelson-maps-in-taquito-8d8be9930662)
...
This contract has a stored counter. The contract is used in some Taquito integration tests for generic tests for features like transfers.
...
Not a supported FA1.2 contract. Almost an FA2 interface but is missing `update_operators`.
...
This contract is another example of a Lambda contract, this time involving a token. It is not a supported FA1.2 contract, but it does have the entrypoints that define an FA2 interface: `balance_of`, `transfer`, and `update_operators`.
...
```js
Tezos.contract
  .at('KT1XUDnehGqGQddfuWA4ffdDMGQEDTk7CzuH')
  .then((myContract) => {
    return myContract
      .storage()
      .then((myStorage) => {
...
Views are meant to be called by a contract using the Michelson Instruction View followed by the view name and its result type. See the [On-chain views documentation](on_chain_views) for more details.
...
```
tezos-client check contract KT1HFpdVddiw5DhCQUXqVJp7WCsy28soWCcH implements fa1.2
```
...
```js
const contract = await Tezos.wallet.at('KT1J3gLYoC7KNWS5h9NXuDjHbR1bFTDdyCNc');
```
...
| Contract                       | Type    | Interface | Metadata | Default Endpoint |
| ------------------------------ | ------- | --------- | -------- | ---------------- |
| IncrementContract              | Basic   |           |          |                  |
| MichelsonMapContract           | Basic   |           |          | default          |
| GenericMultisigContract        | Basic   |           |          | default          |
...
| MapWithMapandBigmapContract    | Maps    |           |          | default          |
| BigMapsMultipleValuesContract  | BigMaps | FA1.2     |          |                  |
| BigMapsComplexStorageContract  | BigMaps |           |          |                  |
| BigMapsWithLedgerContract      | BigMaps |           |          |                  |
| BigMapPackContract             | BigMaps |           |          | default          |
| ContractCallFib                | Views   |           |          | default          |
| ContractTopLevelViews          | Views   |           |          | default          |
| TokenContract                  | Tzip-7  | FA1.2     |          |                  |
| Tzip12BigMapOffChainContract   | Tzip-12 | FA2       | ✓        |                  |
| Tzip16StorageContract          | Tzip-16 |           | ✓        | default          |
| Tzip16HTTPSContract            | Tzip-16 |           | ✓        | default          |
| Tzip16SHA256Contract           | Tzip-16 |           | ✓        | default          |
| Tzip16IPFSContract             | Tzip-16 |           | ✓        | default          |
| Tzip16OffChainContractJSON     | Tzip-16 |           | ✓        | default          |
| Tzip16OffChainContractMultiply | Tzip-16 |           | ✓        | default          |
| WalletContract                 | Wallet  |           |          | default          |
| WalletAreYouThereContract      | Wallet  |           |          |                  |
| SaplingContract                | Sapling |           |          | default          |
