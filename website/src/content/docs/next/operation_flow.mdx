...
Taquito makes injecting operations into the Tezos blockchain very simple by way of three distinct APIs: the Contract API, the Wallet API, and the Batch API.

## Contract API

### What is the Contract API?
The Contract API provides a simple way to interact with the Tezos blockchain. It provides methods and abstractions to prepare, forge, sign, and send operations to the Tezos blockchain, as well as interact with smart contracts.
...
The Wallet API has a few nuances and can get pretty involved to get started and running. So we've provided a separate document with a more in-depth explanation of the Wallet API. You can find that document [here](wallet_API).


## Batch API

There might come a time where you would want to inject multiple operations at the same time. For example, you would like to transfer funds to multiple accounts at once.
...
Making two operations run in this manner will result in an error message, as each Tezos account holds a counter that increments every time an operation is included in a block on the network. This feature prevents users from sending two or multiple transactions at the same time.
...
### Using the Batch API
For a more in-depth explanation of the Batch API, please refer to [this document](batch-api) for examples and use cases that you can adopt into your own projects.

## Interacting Directly With the Flow of Operations
As developers, there might come a time when you need to customize the flow of operations to suit your needs. For example, you would like to modify one of the steps in the flow, or you'd like to have just the prepared object and forge them independently.

Without using the Contract/Wallet API the flow of operations would look something like this:
