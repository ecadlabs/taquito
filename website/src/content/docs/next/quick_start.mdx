...
Details about the smart contract used in this example can be found on tzkt.io [here](https://shadownet.tzkt.io/KT1RNPmCdU9EKRVquuXnVWAbCSQJPbJh2HXc/operations/). To learn more about JSLIGO - a language used to write smart contracts -, see [their documentation](https://ligolang.org/?lang=jsligo).
...
```js live
try {
  const contract = await Tezos.contract.at('KT1RNPmCdU9EKRVquuXnVWAbCSQJPbJh2HXc');
...
```js live wallet
try {
  const contract = await Tezos.wallet.at('KT1RNPmCdU9EKRVquuXnVWAbCSQJPbJh2HXc');
