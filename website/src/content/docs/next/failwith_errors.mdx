---
title: Catching FAILWITH errors
author: Claude Barde
---
...
```ts
try {
    const contractAddress = "KT1Twi2y5n7YuViPqkAtFyPM8hrdpkrsjtC8";
    const contract = await Tezos.contract.at(contractAddress);
    const op = await contract.methodsObject.fail_with_string("a string error").send();
    await op.confirmation();
    console.log(op.hash);
} catch (err) {
    if (err instanceof TezosOperationError) {
      console.log(err.message) // will log "a string error"
    }
}
```
