...
To accept staked funds, a delegate needs to have opted in to staking. This is done by `set_delegate_parameters`. This part is not supported by Taquito, as it is not a dApp operation. More information [here](https://octez.tezos.com/docs/tallinn/staking.html#staking-policy-configuration).

# Staking Funds
To stake your funds, you need to call the `stake` operation.
Before you can stake your funds, two conditions should be met:
- You need to have delegated your funds to a delegate (by calling [`setDelegate`](./set_delegate#setdelegate))
- Your delegate should accept staking by calling `set_delegate_parameters`
...
To unstake your funds, you need to call the `unstake` operation. This will change your funds status to "Unstaked + Frozen"
The unstaked funds will still be frozen (and subject to slashing for 4 cycles). After that, your funds will be in the "Unstaked + Finalizable" state.
...
To finalize your unstaked funds, you need to call the `finalize_unstake` operation. This will change your funds status back to "spendable".
...
## Cycle
A cycle is 14400 blocks with each block being 6 seconds (in the Tallinn protocol). This means a cycle is about 1 day. This is subject to change with the activation of newer protocols.

## Overstaking
A delegate can set the maximum amount of staking they can accept, as a multiple of their own balance. If a delegates limit is exceeded, the exceeding stake is automatically considered as delegation for the delegates baking and voting power calculation, but it does remain slashable. That means it's your responsibility to make sure you're not overstaking. Remember that overstaking can even happen after you have staked successfully if your delegate changes their own balance, or their staking parameters.
...
## Adaptive Issuance
A concept related to staking is adaptive issuance. Adaptive issuance is a mechanism that adjusts the block reward based on the total amount of staked tez. In short, the reward will be adjusted to keep the staked funds about 50% of the total supply. To read more about the mechanism, you can check [this document](https://tezos.gitlab.io/tallinn/adaptive_issuance.html#adaptive-issuance).
...
Before you stake some of your Tez, it's in a "Spendable" state. When you stake it, it will be in a "Staked" state. When you unstake it, it will be in a "Unstaked + Frozen" state, this fund is still subject to slashing. After 4 cycles, it will be in a "Unstaked + Finalizable" state. You can finalize it to make it "Spendable" again.