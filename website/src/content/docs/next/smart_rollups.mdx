...
In Taquito, we have implemented some of the operations in relation to smart rollups. In this document we will go through the operations we support, but we won't go into too much detail on how rollups work behind the scenes. If you'd like to understand the feature a bit deeper, you can refer to [this document](https://tezos.gitlab.io/active/smart_rollups.html).

## `smart_rollup_originate`

The `Smart Rollup Originate` operation allows a user to originate a smart rollup. 

Users must pass a `kernel`, `pvmKind`, and `parameterType`.
...
The add messages operation allows users to send external messages into a rollup inbox.

### Usage
The main use case of sending messages is usually to denote contract calls. These messages usually take the form of this object:
```
MESSAGE='[{\
...
The message includes a `destination`, a `parameter`, and an `entrypoint` property. All components needed to call an entrypoint of a contract.

These messages can then be claimed back into L1 as a legitimate contract call using the `smart_rollup_execute_outbox_message` operation which is explained below.
