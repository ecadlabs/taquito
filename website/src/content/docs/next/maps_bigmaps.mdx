...
This page will help you understand the following actions and concepts:
...
- [Michelson documentation for Map and BigMap](https://docs.tezos.com/smart-contracts/data-types/complex-data-types#big-maps)

## A contract with a single Map for storage

### Origination of the contract with an initial storage

This example builds on the [Ligo Lang Taco Shop learning resources](https://ligolang.org/docs/tutorials/taco-shop/tezos-taco-shop-smart-contract/?lang=jsligo).
...
try {
 const contractOriginated = await Tezos.contract
  .originate({
    ...
    contract = await contractOriginated.contract();
...
try {
  ...
  console.log(`Waiting for confirmation of origination for ${contractOriginated.contractAddress}...`);
  const contract = await contractOriginated.contract();
  console.log(`Origination completed.`);
} catch (error) {
  console.log(`Error: ${JSON.stringify(error, null, 2)}`);
}
...
The example calls the contracts `main` function using the key `1` as its parameter. Remember, we can only change contract storage by calling the function provided by the contract. The `main` function on this Smart Contract is decreasing the value of the `current_stock` associated with the key `1`. We use the `get` method of the `MichelsonMap` class to see the difference in storage after the method call.
...
```js live
try {
  const myContract = await Tezos.contract.at('KT1B3SpFJ1iHagwdkd1utVYP18RyYgZXeGio');
  let myStorage = await myContract.storage();

  //We want to see the value of the key "1"
  let value = myStorage.get('1');
  console.log(
  ...
  const op = await myContract.methodsObject.default('1').send();
  console.log(`Waiting for ${op.hash} to be confirmed...`);
  await op.confirmation(1);
  console.log(`Operation injected.`);

  //Use the get method to see the change in storage
  myStorage = await myContract.storage();
  value = myStorage.get('1');
  console.log(
...
```js live
try {
  const myContract = await Tezos.wallet.at('KT1B3SpFJ1iHagwdkd1utVYP18RyYgZXeGio');
  let myStorage = await myContract.storage();
...
```js live
try {
  const myContract = await Tezos.contract.at('KT1JDDU888CaVWFey2BYxpDzPBaYYn65Q4o3');
...
```js live
try {
  const myContract = await Tezos.wallet.at('KT1JDDU888CaVWFey2BYxpDzPBaYYn65Q4o3');
  const myStorage = await myContract.storage();
  const value = myStorage['theMap'].get({
    0: '2', //nat
    1: 'tz1h3rQ8wBxFd8L9B3d7Jhaawu6Z568XU3xY', //address
...
>[!CAUTION]
> Taquito will handle timestamps with the milliseconds as `2026-09-06T15:08:29.000Z` rather than `2026-09-06T15:08:29Z` which is crucial when accessing maps with complex keys.
...
```js live
try {
  const myContract = await Tezos.contract.at('KT1PBDZULVwbDW7BqEANmyEJGWuzjk43hDJG');
  const myStorage = await myContract.storage();
...
```js live
try {
  const myContract = await Tezos.wallet.at('KT1PBDZULVwbDW7BqEANmyEJGWuzjk43hDJG');
  const myStorage = await myContract.storage();
  const value = myStorage.get({
    0: '1', // int
    1: '2', // nat
...
Map and BigMap are semantically the same except for everything you learned about Maps applies to working with BigMaps. The only difference is that calling `get` on a bigMap will return a Javascript Promise, whereas `get` on a Map returns directly.
...
```js live
try {
  const myContract = await Tezos.contract.at('KT1PZb8sEVvsKQGzLwusyQRkJpSCDQ7WFfny');
  let myStorage = await myContract.storage();
  ...
  const valueBigMap = await myStorage['thebigmap'].get({
    0: '10', //nat
    1: 'tz3WXYtyDUNL91qfiCJtVUX746QpNv5i5ve5', //address
  });
...
```js live
try {
  const myContract = await Tezos.wallet.at('KT1PZb8sEVvsKQGzLwusyQRkJpSCDQ7WFfny');
  let myStorage = await myContract.storage();
...
Taquito allows you to pack the required data locally to fetch values from a big map. By relying on the local pack implementation, Taquito eliminates one RPC roundtrip when fetching big map values. This feature makes fetching big map values approximately **50% faster**.

Implementing this feature is a very easy 2 step process:

1. Import the `MichelCodecPacker` class from `@taquito/taquito`
2. Create an instance of the `MichelCodecPacker` class and pass it to the `setPackerProvider` method of the `TezosToolkit` instance.
...
try {
  const contract = await Tezos.contract.at('KT1LPdW47Aba3kVpNMpRt7sx5yM1M4A8XmAW');
  console.log('Fetching the storage of the contract...');
  const storage = await contract.storage();
  console.log('Fetching the big map values...\n');
  const values = await storage['0'].getMultipleValues([
...
try {
  const contract = await Tezos.wallet.at('KT1LPdW47Aba3kVpNMpRt7sx5yM1M4A8XmAW');
  console.log('Fetching the storage of the contract...');
  const storage = await contract.storage();
  console.log('Fetching the big map values...\n');
  const values = await storage['0'].getMultipleValues([
