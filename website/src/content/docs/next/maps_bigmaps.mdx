...
```js live
try {
  const myContract = await Tezos.contract.at('KT1CVoh9T37gWcurvEoVAYikHdPeQXwV8zQL');
...
```js live
try {
  const myContract = await Tezos.wallet.at('KT1CVoh9T37gWcurvEoVAYikHdPeQXwV8zQL');
  let myStorage = await myContract.storage();
...
```js live
try {
  const myContract = await Tezos.contract.at('KT1Ca2GPgZjVxfExDN3W8ybPHD61t8JLhM4o');
...
```js live
try {
  const myContract = await Tezos.wallet.at('KT1Ca2GPgZjVxfExDN3W8ybPHD61t8JLhM4o');
  const myStorage = await myContract.storage();
  const value = myStorage['theMap'].get({
    0: '2', //nat
    1: 'tz1h3rQ8wBxFd8L9B3d7Jhaawu6Z568XU3xY', //address
...
```js live
try {
  const myContract = await Tezos.contract.at('KT1U7CXVb1wACuhQNVCdFGzv5dv2UkU92kYs');
  const myStorage = await myContract.storage();
...
```js live
try {
  const myContract = await Tezos.wallet.at('KT1U7CXVb1wACuhQNVCdFGzv5dv2UkU92kYs');
  const myStorage = await myContract.storage();
  const value = myStorage.get({
    0: '1', // int
    1: '2', // nat
...
```js live
try {
  const myContract = await Tezos.contract.at('KT1KdnZa8feJS7NBVoDdGt4pGDMvwNTJg8J4');
  let myStorage = await myContract.storage();
...
```js live
try {
  const myContract = await Tezos.wallet.at('KT1KdnZa8feJS7NBVoDdGt4pGDMvwNTJg8J4');
  let myStorage = await myContract.storage();
...
try {
  const contract = await Tezos.contract.at('KT1EFznjrXWY3fPbFvpYnhDeEarsK8Ke9HCB');
...
try {
  const contract = await Tezos.wallet.at('KT1EFznjrXWY3fPbFvpYnhDeEarsK8Ke9HCB');
  console.log('Fetching the storage of the contract...');
  const storage = await contract.storage();
  console.log('Fetching the big map values...\n');
  const values = await storage['0'].getMultipleValues([
