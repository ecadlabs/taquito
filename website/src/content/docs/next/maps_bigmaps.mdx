...
```js live
try {
  const myContract = await Tezos.contract.at('__FIXTURE_mapsTacoShopContract__');
...
```js live
try {
  const myContract = await Tezos.wallet.at('__FIXTURE_mapsTacoShopContract__');
  let myStorage = await myContract.storage();
...
```js live
try {
  const myContract = await Tezos.contract.at('__FIXTURE_mapsPairKeyContract__');
...
```js live
try {
  const myContract = await Tezos.wallet.at('__FIXTURE_mapsPairKeyContract__');
  const myStorage = await myContract.storage();
  const value = myStorage['theMap'].get({
    0: '2', //nat
    1: 'tz1h3rQ8wBxFd8L9B3d7Jhaawu6Z568XU3xY', //address
...
```js live
try {
  const myContract = await Tezos.contract.at('__FIXTURE_mapsEightPairKeyContract__');
  const myStorage = await myContract.storage();
...
```js live
try {
  const myContract = await Tezos.wallet.at('__FIXTURE_mapsEightPairKeyContract__');
  const myStorage = await myContract.storage();
  const value = myStorage.get({
    0: '1', // int
    1: '2', // nat
...
```js live
try {
  const myContract = await Tezos.contract.at('__FIXTURE_mapsMapAndBigMapContract__');
  let myStorage = await myContract.storage();
...
```js live
try {
  const myContract = await Tezos.wallet.at('__FIXTURE_mapsMapAndBigMapContract__');
  let myStorage = await myContract.storage();
...
try {
  const contract = await Tezos.contract.at('__FIXTURE_mapsMultipleBigMapValuesContract__');
...
try {
  const contract = await Tezos.wallet.at('__FIXTURE_mapsMultipleBigMapValuesContract__');
  console.log('Fetching the storage of the contract...');
  const storage = await contract.storage();
  console.log('Fetching the big map values...\n');
  const values = await storage['0'].getMultipleValues([
