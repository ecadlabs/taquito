...
- `arbitrary` is a `bytes` string, with the hex representation of the payload to be signed.
- `basedOnBlock` is a `BlockIdentifier`:

```typescript
type BlockIdentifier = 'head' | `head~${number}` | `B${string}` | number;
```

The payload can be signed on `head`, a specified number of blocks before `head`, a block referenced by hash, or a block referenced by level.
...
Both APIs return a signed object of the form:
