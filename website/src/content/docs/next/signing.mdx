...
A signature is a base58-encoded string whose prefix depends on the key type: `edsig` for ed25519 (tz1), `spsig` for secp256k1 (tz2), `p2sig` for p256 (tz3), and `BLsig` for BLS (tz4). It requires a signer to hash the input bytes and thus can only be done if the signer has access to the private key of the account. Therefore, it is impossible to forge a signature for an account of which you don't have access to the private key. Michelson implements an instruction called `CHECK_SIGNATURE` that verifies a signature was produced by a given public key.
...
You can also sign a string of bytes with a wallet. Unlike the `InMemorySigner`, wallets require a certain format for the bytes that need to be signed. Here is how the string must be formatted:
...
```typescript
import { RequestSignPayloadInput, SigningType } from '@taquito/beacon-wallet/types';
...
### Examples of signing TZIP-32 message with the Contract API and Wallet API
...
try {
  const theSigner = await InMemorySigner.fromSecretKey('edsk2rKA8YEExg9Zo2qNPiQnnYheF1DhqjLVmfKdxiFfu5GyGRZRnb');
...
After forging a signature, you may want to send it to a contract so it can use it within its own logic. 

Let's imagine you have a contract with an entrypoint that accepts a public key, a signature, and bytes called `%check_signature`. Here is how to send it to the contract using Taquito:
