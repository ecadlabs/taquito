...
>[!CAUTION]
> This feature is a work in progress, and may have breaking changes in the future. We encourage Taquito users to try this feature and reach out to us if you have any issues or concerns.
...
try {
  ...
  console.log(`Operation injected: https://ghostnet.tzkt.io/${op.opHash}`);
} catch (error) {
  console.log(error);
}
```
...
Wallet Connect allows listening to specific events. Taquito has listeners on the events `session_delete`, `session_expire`, and `session_update` and will update the internal session member accordingly if one of these events is emitted. The dApp should also handle those events as such:
