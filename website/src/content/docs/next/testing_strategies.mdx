...
You can find more information on software testing philosophies in general [here](https://www.codesimplicity.com/post/the-philosophy-of-testing/).
...
Static Code Analysis is run during the CI/CD cycle to do syntactic checks for errors in the code. Often a line marking a merge conflict or a violation of a coding format will cause a static analyzer to complain. During a CI/CD run, a Pull Request will be examined by [CodeQL](https://codeql.github.com/) and [Snyk](https://snyk.io/).
...
The Taquito test suite will run tests in CI/CD against the current and next test network. There is also testing of “Weeklynet,” which represents the bleeding edge of the available Tezos test code.
