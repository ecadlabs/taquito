...
Its integration into the main `Taquito` package makes it easier to write storage when deploying a contract and parameters when calling a contract entry-point.
...
or
```js
const script = await Tezos.rpc.getScript('KT1J4E79F1qL6kGBSQ3yXBdXmuq5j4FNThK2');
...
Here is an example with a contract storage having annotations and a `pair` at its root. The `generateSchema` method returns an object where the keys are the annotations, and the values show the corresponding type.
...
We can use the `Typecheck` method to validate the storage object. The method takes the storage object as a parameter and returns `true` if the storage object is compliant with the schema type or `false` otherwise. 

Validation of the properties is performed. For example, the key used in the following code snippet is invalid, so the returned value of the `Typecheck` method is false.
...
If we have a contract with a big map in its storage, when we fetch the contract's storage with the RPC, the returned value looks like the following `{ int: big_map_id }`.
...
const dataMichelson = {"prim":"Pair","args":[{"string":"KT1LAbD14CzzK88e67hpyiCfrB3XgfPVku68"},{"string":"test"},{"int":"2"}]}
...
Formally speaking, the fields are the leaves of the tree, ordered by traversing the tree (because we omit inner nodes of the tree, there is no difference between pre-order, in-order, or post-order traversal).
