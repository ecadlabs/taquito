...
```js live
try {
  const contract = await Tezos.contract.at('KT1RsX9ymtbYyfxP97EVtYSJ97N33CLsVvJk');
...
```js live
try {
  const contract = await Tezos.contract.at('KT1X8ofXHePk5AKoBRaZzWmtKiXYtiKi2zwQ');
