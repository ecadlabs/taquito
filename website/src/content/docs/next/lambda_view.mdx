...
```js live
try {
  const contract = await Tezos.contract.at('KT1MhfAnNbg2oACFBP4VDU5bNY5MZUXdeDWs');
...
```js live
try {
  const contract = await Tezos.contract.at('KT1Ccr6ZMeB1mp9yJAqJTHK7F4xoFV9uc11T');
