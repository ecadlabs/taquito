...
If you plan to use Temple wallet directly (not through Beacon), you will also need `@temple-wallet/dapp`.
...
The `BeaconWallet` is a package implementing the TZIP-10 standard that describes the communication between a dapp (decentralized application on Tezos) and a wallet (e.g., a browser extension). The Beacon wallet works with any wallet that supports the TZIP-10 standard (for example, Temple or Kukai). This package is the recommended way of connecting your dapp to a wallet. In addition to being future-proof, it gives your users the freedom to choose the wallet they want.
...
The necessary bare minimum to instantiate the wallet is an object with a `name` property that contains the name of your dApp and the network you want it to point to. In this case, we choose to point it to `shadownet`.

Beacon wallet allows you to customize your dApp responses to different events. In the example above, instead of getting the default Beacon pop-up after the user connects the wallet, it will display the available data in the console. You can use whatever solution you prefer for feedback. The full set of events is described by the `BeaconEvent` enum re-exported from `@taquito/beacon-wallet` (backed by the [Octez Connect](https://docs.tezos.com/dApps/migrating-from-beacon) SDK, formerly Beacon SDK).
...
You can choose among `mainnet`, `shadownet`, `tallinnnet`, `tezlink_shadownet`, or `custom` to set up the network. Once the permissions have been configured, you can get the user's address by calling the `getPKH` method on the wallet:
...
Make sure you have a TZIP-10 compatible wallet installed (e.g., Temple browser extension or Kukai web wallet). The `BeaconWallet` works with any wallet that implements the TZIP-10 standard.
...
The different types of events are defined in the type `BeaconEvent` that can be imported from `"@taquito/beacon-wallet"`. Some of the events are: `ACTIVE_ACCOUNT_SET`, `PAIR_SUCCESS`, `SIGN_REQUEST_SUCCESS`.
...
Taquito provides two methods for ending a wallet session. Choosing the wrong one can leave stale Beacon state in localStorage that causes connection failures later.

#### `wallet.disconnect()` — Full logout (recommended)

Removes **all** Beacon data from localStorage, including the active account, cached relay node, and peer information. The BeaconWallet instance is no longer usable after calling this method; you must create a new one to reconnect.

Use this whenever a user logs out of your dApp.

```ts
// Full logout — clears all beacon:* localStorage keys
await wallet.disconnect();

// The wallet instance is now unusable. To reconnect:
const wallet = new BeaconWallet({ name: 'MyDapp' });
await wallet.requestPermissions({ network: { type: NetworkType.MAINNET } });
```

#### `wallet.clearActiveAccount()` — Account switching only

Removes only the active account reference. Other Beacon state (relay node, peer data) is preserved. Use this when switching between accounts within the same session.

```ts
// Switch accounts — keeps the Beacon session alive
await wallet.clearActiveAccount();
await wallet.requestPermissions({ network: { type: NetworkType.MAINNET } });
```

>[!CAUTION]
> Do not use `clearActiveAccount()` as a logout. It leaves `beacon:matrix-selected-node` in localStorage. If that relay node later goes offline, users will be unable to connect until they manually clear their browser storage.
...
Then, Temple requires an extra step to verify that the extension is installed and connected in the browser. The `TempleWallet` class exposes a static property called `isAvailable` that just does that. You must use it before attempting to connect the wallet:
...
```js
await wallet.connect('mainnet' | 'tallinnnet' | 'shadownet' | 'sandbox');
```
...
try {
  const op = await Tezos.wallet
    .transfer({ to: '__FIXTURE_walletTransferContract__', amount: 0.2 })
    .send();
...
```js
const contract = await Tezos.wallet.at('KT1SwCVCPfcYaB3K8Xab4wAjrYSRAVqw9Bbg');
```
...
```js live wallet
try {
  const contract = await Tezos.wallet.at('__FIXTURE_walletEntrypointsContract__');
...
4. `pkh`: retrieves the public key hash of the account that is currently in use by the wallet
