...
You can choose among `mainnet`, `ghostnet` (deprecated, shutting down end of March 2026), `shadownet`, `tallinnnet`, `tezlink_shadownet`, or `custom` to set up the network. Once the permissions have been configured, you can get the user's address by calling the `getPKH` method on the wallet:
...
```js
await wallet.connect('mainnet' | 'ghostnet' | 'tallinnnet' | 'shadownet' | 'sandbox');
```
