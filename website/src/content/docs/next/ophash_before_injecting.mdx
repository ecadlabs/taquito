...
There might come a time when you would like to retrieve the operation hash without injecting it into the blockchain.

To accomplish this, we have a utility function called `encodeOpHash()` in the `@taquito/utils` package. This function takes in the signed operation bytes in the form of a `string`, and outputs the operation hash of said transaction without injecting it.
