...
- **`minimalFeeMutez`**: Minimum fees for the operation according to baker defaults.
- **`storageLimit`**: The limit on the amount of storage an operation can use.
- **`suggestedFeeMutez`**: The suggested fee for the operation, including minimal fees and a small buffer.
- **`totalCost`**: The sum of `minimalFeeMutez` + `burnFeeMutez`.
- **`usingBaseFeeMutez`**: Fees according to your specified base fee, ensuring that at least minimum fees are used.
