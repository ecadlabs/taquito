...
Sapling was introduced to Tezos in the Edo protocol. Refer to the Tezos documentation for more information on Sapling: https://tezos.gitlab.io/active/sapling.html

>[!NOTE]
> We use the term "shielded set" rather than "shielded pool" in our documentation. While both terms refer to the collection of shielded assets in a Sapling contract, "shielded set" more accurately reflects that assets are kept separately with only amounts and ownership identities being private on-chain, rather than being commingled together. This distinction is important for accurate understanding of how Sapling privacy works. See the [Tezos Agora discussion](https://forum.tezosagora.org/t/growing-the-shielded-set-on-tezos/6922) for more context.

## Runtime setup

Viewing-only Sapling flows do not need any parameter initialization. You can derive keys, addresses, inspect Sapling state, and read balances or history without calling any setup function.

Proof-generating flows are different. Shielding, private transfers, and unshielding need the Sapling proving parameters, and Taquito now treats those parameters as runtime resources instead of bundle-time assets.

Use `initSapling()` to select the parameter source policy up front:
...
`initSapling()` does not eagerly download the proving parameters. It only fixes the source policy for the current process. The first proving flow lazy-loads the parameters on demand.

If you want to hide first-use latency, call `preloadSaplingParams()` once after `initSapling()`:
...
## Parameter sources and integrity

Taquito supports four parameter source modes:

- `source: 'taquito'`, the default first-party mirror
- `source: 'zcash'`, which uses the Zcash-hosted mirror
- custom remote URLs with explicit SHA-256 digests
- local filesystem paths for Node.js and CI

Remote loading is always hash-verified before the parameters are used. The digest is the trust anchor, not the hostname.

Custom remote mirrors must include explicit digests:

```ts
await initSapling({
  params: {
    spendParamsUrl: 'https://example.com/sapling-spend.params',
    outputParamsUrl: 'https://example.com/sapling-output.params',
    spendParamsSha256: '8e48ffd23abb3a5fd9c5589204f32d9c31285a04b78096ba40a79b75677efc13',
    outputParamsSha256: '2f0ebbcbb9bb0bcffe95a397e7eba89c29eb4dde6191c339db88570e3f3fb0e4',
  },
});
```

For hermetic Node.js or CI environments, local paths are supported and digests are optional:

```ts
await initSapling({
  params: {
    spendParamsPath: '/absolute/path/to/sapling-spend.params',
    outputParamsPath: '/absolute/path/to/sapling-output.params',
  },
});
```

If you provide local digests, Taquito verifies them. If you omit them, the trust boundary becomes the local filesystem or mounted artifact store.

## Browser deployment

The supported browser contract is:

- `@taquito/sapling` stays ESM-first for application code.
- `@taquito/sapling-wasm` ships the Wasm binary in npm.
- proving parameters are fetched lazily at runtime instead of being embedded in the normal browser import path.

This keeps initial Sapling page loads smaller and removes the need for third-party ESM rewriting or browser `require()` shims.
...
The proving key can be used to generate proof without allowing tokens to be spent. 

Zero-knowledge proofs can be created with either the spending or the proving key. In cases where the device holding the spending key is computationally or memory-limited, such as a hardware wallet, proofs can be produced on a separate device using the proving key.
...
Refer to the following link for more information: [InMemoryViewingKey](./sapling_in_memory_viewing_key)

**Sapling address**

Sapling addresses are used to receive tokens. They are derived from the viewing key.

Here is an example on how to retrieve addresses: [InMemoryViewingKey](./sapling_in_memory_viewing_key#how-to-retrieve-payment-addresses-from-the-viewing-key)
...
const saplingContract = await tezos.contract.at('KT18iaRnYYNvEJpnk6e46NwBGKNJ8Dn19mtv');
...
## How to retrieve my balance in the Sapling shielded set?
...
const saplingToolkit = new SaplingToolkit(
    { saplingSigner: inMemorySpendingKey },
    { contractAddress: 'KT18iaRnYYNvEJpnk6e46NwBGKNJ8Dn19mtv', memoSize: 8 },
    readProvider
);

saplingToolkit.getSaplingTransactionViewer()
  .then((txViewer) => {
    console.log(`Fetching Alice balance in the shielded set...`);
...
saplingToolkit.getSaplingTransactionViewer()
  .then((txViewer) => {
    console.log(`Fetching Alice's history of transactions in the shielded set...`);
...
The `prepareShieldedTransaction` method returns the crafted Sapling transaction parameter but does not perform any change on the shielded set. A subsequent step where the Sapling transaction parameter is submitted to the smart contract must be done. Note that in a case of a shielded transaction, the shielded amount must be sent along when calling the smart contract to transfer the tez to the shielded set, or it will result in an error.
...
const saplingContractAddress = 'KT18iaRnYYNvEJpnk6e46NwBGKNJ8Dn19mtv'
...
inMemorySpendingKey.getSaplingViewingKeyProvider()
  ...
  .then((shieldedTx) => {
    ...
    .then((hash) => console.log(`Operation injected: https://shadownet.tzkt.io/${hash}`))
  })
...
The `prepareSaplingTransaction` method returns the crafted Sapling transaction parameter but does not perform any change on the shielded set. A subsequent step where the Sapling transaction parameter is submitted to the smart contract must be done.
...
The `prepareUnshieldedTransaction` method returns the crafted Sapling transaction parameter but does not perform any change on the shielded set. A subsequent step where the Sapling transaction parameter is submitted to the smart contract must be done to retrieve the tokens from the shielded set.
...
Refer to these sections to [retrieve the balance](sapling#how-to-retrieve-my-balance-in-the-sapling-shielded-set) and [view the transaction history](sapling#how-to-retrieve-my-transaction-history).
