...
1. First, create an instance of `Tzip12Module` and add it as an extension to our `TezosToolkit`
...
const contractAddress = "__FIXTURE_tzip12OffChainViewContract__";
const tokenId = 1;

try {
  const contract = await Tezos.contract.at(contractAddress, compose(tzip12, tzip16));
  console.log(`Fetching the token metadata for the token ID ${tokenId}...`);
  const tokenMetadata = await contract.tzip12().getTokenMetadata(tokenId);
...
try {
  const wallet = await Tezos.wallet.at(contractAddress, compose(tzip12, tzip16));
  console.log(`Fetching the token metadata for the token ID ${tokenId}...`);
  const tokenMetadata = await wallet.tzip12().getTokenMetadata(tokenId);
  console.log(JSON.stringify(tokenMetadata, null, 2));
} catch (error) {
  console.log(`Error: ${JSON.stringify(error, null, 2)}`);
}
...
try {
  ...
  const result = await views['token_metadata']().executeView(tokenId);
...
>[!NOTE]
> To be [TZIP-12 compliant](https://gitlab.com/tzip/tzip/-/blob/master/proposals/tzip-12/tzip-12.md#token-metadata-storage-access), the type of the big map `%token_metadata` in the storage of the contract should match the following type, otherwise the token metadata won't be found by the `getTokenMetadata` method, and a `TokenMetadataNotFound` error will be thrown:
...
try {
  const wallet = await Tezos.wallet.at(contractAddress, tzip12);
  console.log(`Fetching the token metadata for the token ID ${tokenId} of ${contractAddress}...`);
  const tokenMetadata = await wallet.tzip12().getTokenMetadata(tokenId);
  console.log(JSON.stringify(tokenMetadata, null, 2));
} catch (error) {
