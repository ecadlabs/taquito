...
You can choose among `mainnet`, `seoulnet`, `tallinnnet`, `ghostnet` and `custom` to set up the network. Once the permissions have been configured, you can get the user's address by calling the `getPKH` method on the wallet:
...
```js
await wallet.connect('mainnet' | 'seoulnet' | 'tallinnnet' | 'ghostnet' | 'mondaynet' | 'sandbox');
```
