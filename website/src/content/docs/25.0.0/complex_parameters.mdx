...
In the example below we originate the contract with initial values in the storage. We use the `MichelsonMap` class' of Taquito to initialize [the maps and the bigMap](maps_bigmaps).

As described above, the `map %data` uses a value that we chose between different types. When using Taquito, we need to surround the chosen argument with curly braces. In the example below, we initialize the value in the `map %data` to the boolean true: `{ bool: true }`.
...
try {
  ...
  const contractOriginated = await Tezos.contract.originate({
    code: contractJson,
    storage: {
      owner: 'tz1PgQt52JMirBUhhkq1eanX8hVd1Fsg71Lr',
...
try {
  await importKey(Tezos, secretKey);
  const myContract = await Tezos.contract.at('__FIXTURE_complexParametersContract__');
...
try {
  ...
  const op = await myContract.methodsObject.set_child_record({
    address: { Some: 'tz1PgQt52JMirBUhhkq1eanX8hVd1Fsg71Lr' },
    data: dataMap,
    label: 'EEEE',
    owner: 'tz1PgQt52JMirBUhhkq1eanX8hVd1Fsg71Lr',
  ...
  console.log(`Operation injected: https://shadownet.tzkt.io/${op.hash}/operations`);
} catch (error) {
  console.log(`Error: ${JSON.stringify(error, null, 2)}`);
}
```
