...
```js live noConfig
Tezos.setRpcProvider('https://tezos-shadownet.octez.io/');

try {
  const walletConnect = await WalletConnect.init({
    logger: 'debug',
    projectId: 'ba97fd7d1e89eae02f7c330e14ce1f36',  // demo project ID for this example; get your own from https://cloud.reown.com
    metadata: {
  ...
  console.log('Waiting for WalletConnect pairing approval...');
  await walletConnect.requestPermissions({
    permissionScope: {
      networks: [NetworkType.SHADOWNET],
  ...
  console.log('WalletConnect pairing approved, preparing transfer...');
  ...
  console.log(`Operation injected: https://shadownet.tzkt.io/${op.opHash}`);
} catch (error) {
  console.log(error);
}
```
