...
import { docsLiveCodeFixtures as fixtures } from '../../../utils/live-code.mjs';

## Installing Taquito using npm

> If you just want to get started quickly with a new project, you can use our boilerplate [Vue](https://github.com/ecadlabs/taquito-vue-template), [React](https://github.com/ecadlabs/taquito-react-template), or [Ionic](https://github.com/ecadlabs/taquito-ionic-template) templates.
...
try {
  ...
  console.log(`Operation injected: https://shadownet.tzkt.io/${hash}/operations`)
} catch (error) {
...
try {
  ...
  console.log(`Operation injected: https://shadownet.tzkt.io/${confirmation.block.hash}/operations`)
} catch (error) {
  console.log(`Error: ${error} ${JSON.stringify(error, null, 2)}`);
}
```
...
Details about the smart contract used in this example can be found on tzkt.io <a href={`https://shadownet.tzkt.io/${fixtures.quickStartIncrementContract}/operations/`}>here</a>. To learn more about JSLIGO, a language used to write smart contracts, see [their documentation](https://ligolang.org/?lang=jsligo).
...
```js live wallet
try {
  const contract = await Tezos.wallet.at('__FIXTURE_quickStartIncrementContract__');
