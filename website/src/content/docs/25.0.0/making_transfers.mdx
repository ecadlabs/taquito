...
```js live
try {
  console.log(`Configuring a fresh demo signer...`);
  const address = await configureSigner({ fresh: true, minBalanceMutez: 2_000_000 });

  if (!address) {
    throw new Error('Could not configure a demo signer');
  }

  const amount = 0.5;
  const recipient = 'tz1h3rQ8wBxFd8L9B3d7Jhaawu6Z568XU3xY';

  console.log(`Transfering ${amount} ꜩ from ${address} to ${recipient}...`);
  const operation = await Tezos.contract.transfer({ to: recipient, amount });
...
Sending 1 mutez to `KT1J4E79F1qL6kGBSQ3yXBdXmuq5j4FNThK2` from `KT1...`

```js
const contract = await Tezos.contract.at('KT1...');
await contract.methodsObject
  .do(transferToContract('KT1J4E79F1qL6kGBSQ3yXBdXmuq5j4FNThK2', 1))
  .send({ amount: 0 });
```
