---
title: Changelog
description: Release notes and version history for Taquito, the TypeScript library for Tezos blockchain development. Track new features, bug fixes, and breaking changes.
author: Jev Bjorsell
layout: ../layouts/MarkdownLayout.astro
---
# Taquito (unreleased)

## Wallet SDK migrated to Octez Connect

**TL;DR:** `@taquito/beacon-wallet` now depends on the renamed **Octez Connect** packages (`@tezos-x/octez.connect-*`) at `4.8.5` instead of the `@ecadlabs/beacon-*` fork. This is the upstream rename of the Beacon SDK (see the [migration guide](https://docs.tezos.com/dApps/migrating-from-beacon)). The public API of `@taquito/beacon-wallet` is unchanged — the `BeaconWallet` class and the re-exported `BeaconEvent`, `NetworkType`, `PermissionScope`, `SigningType`, etc. keep the same names and import paths — so **most dApps need no code changes** beyond reinstalling.

### Upgrade checklist

- If you import wallet SDK symbols **through `@taquito/beacon-wallet`** (recommended — e.g. `import { NetworkType, BeaconEvent } from '@taquito/beacon-wallet'`), no change is needed.
- If you depended on `@ecadlabs/beacon-*` (or the original `@airgap/beacon-*`) packages **directly**, replace them with the corresponding `@tezos-x/octez.connect-*` package at `4.8.5`:
...
- **Remove any `@airgap/beacon-*` (or `@ecadlabs/beacon-*`) `overrides` from your `package.json`** — they are no longer needed. This supersedes the guidance in the v24.3.0 entry below, which said to keep the `@airgap/beacon-*` overrides; with this release those overrides should be deleted.
- If your bundler config (Vite, webpack, etc.) references the SDK packages by name in `manualChunks`, `externals`, or chunk splitting, update those references from `@ecadlabs/beacon-*` / `@airgap/beacon-*` to `@tezos-x/octez.connect-*`.

---
# Taquito v24.3.0

## Network reliability, browser compatibility, and internal modernization

**TL;DR:** Node 22 is the new minimum for dev and server-side use. Ghostnet has been retired, migrate to Shadownet. The Beacon `NetworkType` enum has been trimmed of 15 dead protocol testnets (breaking for anyone who imported them). Gas estimation now matches `octez-client` semantics on mainnet. `@taquito/sapling` no longer runs a `postinstall` network fetch. Beacon defaults now route through the Trilitech-operated `octez.io` relay nodes via our `@ecadlabs/beacon-*` fork. Several browser compatibility fixes for bundled dApps.
...
- **Node.js >=22 is now required for development and server-side use.** Node 20 reaches EOL April 2026; Node 22 ships with native `globalThis.fetch`, which let us drop `node-fetch`. This affects build toolchains, CI pipelines, and applications that import Taquito server-side. Bundled browser applications are unaffected.
...
- **`bignumber.js` upgraded to v10.** Taquito normalizes at its own API boundary, so this is transparent for most consumers. If your code names `BigNumber` in its own type signatures, add a direct `bignumber.js@^10` dependency to avoid TypeScript treating the transitive copy as a different type. ([#3383](https://github.com/ecadlabs/taquito/pull/3383))
- **Beacon `NetworkType` enum trimmed (breaking for some).** Fifteen long-retired Tezos protocol testnets (`DELPHINET`, `EDONET`, `FLORENCENET`, `GRANADANET`, `HANGZHOUNET`, `ITHACANET`, `JAKARTANET`, `KATHMANDUNET`, `LIMANET`, `MUMBAINET`, `NAIROBINET`, `OXFORDNET`, `PARISNET`, `QUEBECNET`, `RIONET`) have been removed from `NetworkType`. If your code imports any of these via `@taquito/beacon-wallet/types`, TypeScript will fail until you delete the references. `GHOSTNET` and `SEOULNET` are now `@deprecated` aliases, migrate to `SHADOWNET` and `TALLINNNET` respectively; both will be removed in a future release. New: `TEZOSX_PREVIEWNET` and `USHUAIANET`. See the Beacon SDK fork section below.
- **`@taquito/sapling` no longer uses `postinstall`.** The install-time network fetch was a supply-chain risk: installs depended on external availability, and the fetched parameters were not integrity-checked. Proving parameters are now lazy-loaded at runtime via `initSapling()`, SHA-256 verified before use regardless of source, and served from a first-party mirror at `sapling.taquito.io` by default. Viewing-only flows (key derivation, balances, history) need no initialization. If you want to hide first-use latency, call `preloadSaplingParams()` after init. The wasm dependency moved from `@airgap/sapling-wasm` to `@taquito/sapling-wasm@0.2.0`. Four source modes are supported: `'taquito'` (default), `'zcash'`, custom remote URLs with SHA-256 digests, and local filesystem paths for Node.js/CI. If you don't use sapling, none of this affects you. Thanks to the [ShieldBridge](https://shieldbridge.xyz/) team for feedback on the sapling runtime changes.
...
If you use an AI coding assistant (Claude Code, Cursor, Copilot, etc.), you can paste the following prompt into your project to assist with the upgrade. It is scoped to the changes in this release. Review and test the changes before committing.
...
Public references: [#3352](https://github.com/ecadlabs/taquito/issues/3352), [#3353](https://github.com/ecadlabs/taquito/pull/3353)
...
Exact read context is propagated through contract, big map, sapling, and TZIP-16 read paths. Shadownet secret-key integration runs have also been normalized to the same rolling RPC as the ephemeral shadownet config. ([#3381](https://github.com/ecadlabs/taquito/pull/3381))
...
### Beacon SDK fork (4.8.1-ecad.7)

Taquito now depends on `@ecadlabs/beacon-*` `4.8.1-ecad.7`, published from our `taquito-patches` maintenance line. This package is effectively a fork of both the upstream Papers-maintained Beacon SDK and Trilitech's `octez.connect`, combining selected backports from `octez.connect` with ECAD-maintained fixes on top of the `4.8.1` base. For background on the ECAD-maintained Beacon package line, see the [`beacon-sdk-taquito-patches` README](https://github.com/ecadlabs/beacon-sdk-taquito-patches/blob/taquito-patches/README.md).

**Tezos network surface cleanup (breaking).** `NetworkType` has been trimmed of 15 long-deactivated protocol testnets. If your dApp or wallet imports any of `DELPHINET`, `EDONET`, `FLORENCENET`, `GRANADANET`, `HANGZHOUNET`, `ITHACANET`, `JAKARTANET`, `KATHMANDUNET`, `LIMANET`, `MUMBAINET`, `NAIROBINET`, `OXFORDNET`, `PARISNET`, `QUEBECNET`, or `RIONET` from `@taquito/beacon-wallet/types`, the TypeScript compile will fail. Delete the references. `GHOSTNET` and `SEOULNET` are now `@deprecated`, migrate to `SHADOWNET` and `TALLINNNET` respectively. New: `TEZOSX_PREVIEWNET` (TzKT: `tzkt.previewnet.tezosx.nomadic-labs.com`) and `USHUAIANET` (TzKT: `ushuaianet.tzkt.io`). `BlockExplorer.rpcUrls` type changed from `{ [key in NetworkType]: string }` to `{ [key in NetworkType]?: string }` so custom subclasses no longer have to enumerate every network; `getLinkForNetwork` now throws a clear error when a network has no configured URL instead of returning `undefined`.

**`axios` removed from the Beacon SDK fork.** The Matrix transport HTTP client, `WalletClient` push-notification oracle, and `DAppClient` notification send were rewritten on top of native `fetch` + `AbortController`. Public API contracts are preserved, and error semantics for callers that inspect Matrix `errcode` bodies on rejection are preserved. This closes [GHSA-r4q5-vmmm-2653](https://github.com/advisories/GHSA-r4q5-vmmm-2653) (`follow-redirects` leaks custom auth headers to cross-domain redirect targets) at the source, and drops nine packages from the lockfile transitive closure (`axios`, `follow-redirects`, `form-data`, `proxy-from-env`, plus helpers).
...
Compared with `4.8.1-ecad.2`, the broader ecad.6/ecad.7 line also pulled in improved Matrix relay reliability and wallet extension detection from `octez.connect`, moved the default Beacon/Matrix relay network from the old `papers.tech` nodes to the Trilitech-operated `octez.io` nodes, and refreshed outdated dependencies.

If dApps still see wallet connection failures at the relay layer after upgrading, those issues live downstream of Taquito and should be directed to [`octez.connect`](https://github.com/trilitech/octez.connect/issues).
...
- The TZIP-16 metadata URI parser was rewritten to eliminate a CodeQL-flagged ReDoS pattern. ([#3385](https://github.com/ecadlabs/taquito/pull/3385))
- `@taquito/beacon-wallet` package `exports` conditions were reordered to fix bundler resolution. ([#3390](https://github.com/ecadlabs/taquito/pull/3390))

### Browser compatibility

- `@taquito/michelson-encoder` no longer relies on Node.js `Buffer` for hex conversion. BLS12-381, chest, chest-key, bytes, and sapling-transaction token types now use `buf2hex` from `@taquito/utils`. ([#3400](https://github.com/ecadlabs/taquito/pull/3400))
- `@taquito/utils` dropped `typedarray-to-buffer` in favor of `Buffer.from()` in `b58Encode`. ([#3400](https://github.com/ecadlabs/taquito/pull/3400))
- `@taquito/sapling` now explicitly imports the `buffer` polyfill and re-exports `@taquito/sapling-wasm` through a shim that ensures `globalThis.Buffer` is available. ([#3400](https://github.com/ecadlabs/taquito/pull/3400))
- `process.env` access in `@taquito/http-utils` and `@taquito/taquito` is now guarded behind `typeof process !== 'undefined'`, preventing crashes in browser environments that don't shim `process`. ([#3400](https://github.com/ecadlabs/taquito/pull/3400))
- `crypto-browserify` was dropped from `@taquito/beacon-wallet` dependencies. ([#3394](https://github.com/ecadlabs/taquito/pull/3394))
- Browser smoke coverage now includes a hosted Sapling parameter preload flow against `sapling.taquito.io`, and obsolete browser shim paths were removed.
- A browser package smoke test suite now covers import and basic instantiation for all published packages under Vite/esbuild. ([#3400](https://github.com/ecadlabs/taquito/pull/3400))
...
- Bundler and package compatibility improved, especially around Vite, esbuild, ESM resolution, and dependency declarations. The install-time Angular patch (a workaround for a legacy peer-dependency conflict) was removed, simplifying package installation.
- The shipped-package toolchain was refreshed across Rollup, Vitest, Ledger transport, and related build helpers. Several unused transitive dependencies (`whatwg-url`, `request`, `tmp`, `lodash` paths) were dropped from the root lockfile, and legacy `signature.json` files were removed from published package manifests. ([#3379](https://github.com/ecadlabs/taquito/pull/3379), [#3386](https://github.com/ecadlabs/taquito/pull/3386), [#3387](https://github.com/ecadlabs/taquito/pull/3387))
...
- Release version-sync now only rewrites dependency ranges for actual workspace packages, preserving external scoped packages like `@taquito/sapling-wasm`. Release and rehearsal workflows install the stamped graph before version-stamp, build, test, and publish. ([#3391](https://github.com/ecadlabs/taquito/pull/3391))
- Documentation and website content received a broad cleanup pass, including `SECURITY.md`, root `README.md`, and website doc drift. Security reporting now routes through GitHub private vulnerability reporting with email fallback. A repository CODEOWNERS file was added for `@ecadlabs/taquito-team`. ([#3386](https://github.com/ecadlabs/taquito/pull/3386), [#3388](https://github.com/ecadlabs/taquito/pull/3388))
- Website live code examples and Ledger documentation were fixed for browser compatibility. ([#3404](https://github.com/ecadlabs/taquito/pull/3404), [#3405](https://github.com/ecadlabs/taquito/pull/3405))

---

### Sustaining Taquito

Taquito is now funded entirely by ECAD Labs. Each release is the product of ongoing maintenance work that rarely shows up in a feature list: fork coordination (see the Beacon SDK fork section above), relay migrations, gas estimation investigations, hosting first-party Sapling parameters, browser compatibility regressions, toolchain and security updates, and responding to issues from teams integrating Taquito into production systems.
...
Report bugs on [GitHub](https://github.com/ecadlabs/taquito/issues). Questions? Join [@tezostaquito](https://t.me/tezostaquito) on Telegram.

---

# Taquito v24.2.0

## Fixes wallet connection freezes, simplifies Beacon imports

**TL;DR:** Wallet connections that freeze on page load are fixed (affected ~33% of users). You can now import `NetworkType`, `BeaconEvent`, etc. from `@taquito/beacon-wallet` instead of depending on Beacon SDK packages directly. Update your imports and remove `@airgap/beacon-*` from your `package.json`.

---

### Fixes wallet connections that freeze on page load

If your dApp's wallet connection has been hanging indefinitely on page load, with no way to recover except manually clearing site data, this release fixes it. We estimate roughly 33% of users have been affected, corresponding to those with established sessions on relay servers that were decommissioned during the Octez.connect & Papers node transition/reorganization.

This beta ships with a patched Beacon SDK fork maintained by ECAD Labs ([ecadlabs/beacon-sdk-taquito-patches](https://github.com/ecadlabs/beacon-sdk-taquito-patches)), published as `@ecadlabs/beacon-*` on npm. The Beacon SDK is outside the scope of Taquito and not maintained by the Taquito team, but these bugs are urgent enough that we've taken matters into our own hands.

Fixes are available to the AirGap, Beacon SDK, and Octez.connect teams. They will be dropped from the fork if and when upstream merges and adequately tests them.

**What was happening:** During the Octez.connect transition, relay servers were decommissioned while new ones were added, with insufficient testing. Users who had established sessions on the old servers got stuck in a deadlock: the SDK tries to reconnect to a server that no longer exists, an internal promise never resolves, and every subsequent call hangs permanently. The wallet can't recover or re-pair. The only workaround was clearing site data.

**What we fixed:**

- **10s timeout on relay server health checks** so dead servers fail fast instead of hanging indefinitely
- **Automatic recovery from stale servers**: when a stored server is unreachable, it's removed from storage and the SDK discovers a working one
- **No more hung promises**: concurrent callers get a proper error instead of waiting forever
- **Connections route to working servers**: Taquito injects known-good relay nodes operated by Papers and Trilitech, replacing decommissioned ones

### Beacon SDK types re-exported from `@taquito/beacon-wallet`

Until now, using `@taquito/beacon-wallet` for basic things like setting a network type or subscribing to wallet events required installing and importing directly from Beacon SDK packages. That couples your app to whichever Beacon implementation Taquito happens to use internally, and the underlying package name has already changed across releases.

`@taquito/beacon-wallet` now re-exports the Beacon SDK types that dApps actually use, so you no longer need a direct dependency on any Beacon package.

**Side-effect-free types** (enums, interfaces) are available from the `./types` subpath:

```typescript
import { NetworkType, SigningType, PermissionScope } from '@taquito/beacon-wallet/types';
import type { RequestSignPayloadInput } from '@taquito/beacon-wallet/types';
```

**`BeaconEvent` and `DAppClientOptions`** are re-exported from the main entry point (they live in `beacon-dapp` which has unavoidable side effects):

```typescript
import { BeaconWallet, BeaconEvent } from '@taquito/beacon-wallet';
import type { DAppClientOptions } from '@taquito/beacon-wallet';
```

**Dynamic imports** also work with the new paths:
...
```diff
 {
   "dependencies": {
     ...
     "@taquito/beacon-wallet": "^24.2.0-beta.1",
-    "@airgap/beacon-dapp": "...",
-    "@airgap/beacon-sdk": "...",
-    "@airgap/beacon-types": "..."
   }
...
If you had `@airgap/beacon-*` overrides in your `package.json`, those can also be removed.

#### Build config updates

If your Vite, webpack, or other bundler config references `@airgap/beacon-*` packages by name (e.g., in `manualChunks`, `externals`, or chunk splitting), update those references. The underlying packages are now `@ecadlabs/beacon-*`:
...
Note: there is no `@ecadlabs/beacon-sdk` meta-package. If you referenced `@airgap/beacon-sdk` in your build config, drop it or replace with the specific packages above.

#### Who must migrate now

If you use pnpm strict mode or Yarn PnP, importing from undeclared transitive dependencies will fail. Switch to the new paths.

#### Who should migrate now

Everyone else. Your current imports still work under npm's flat hoisting, but if the underlying Beacon package ever changes again, you'd need to update all your imports a second time. Do it once now and you're insulated.

#### Full re-export reference

| Symbol | Import from |
|--------|-------------|
| `NetworkType`, `SigningType`, `PermissionScope`, `Regions` | `@taquito/beacon-wallet/types` |
| `RequestPermissionInput`, `RequestSignPayloadInput`, `NodeDistributions` (type-only) | `@taquito/beacon-wallet/types` |
| `BeaconEvent` | `@taquito/beacon-wallet` |
| `DAppClientOptions` (type-only) | `@taquito/beacon-wallet` |

If you import other Beacon SDK types not listed here (e.g., `ExtendedPeerInfo`, `PeerInfo`), those are not re-exported. You'll need to import them from the underlying Beacon package directly, but consider whether your code actually needs them.

#### Migration assistant prompt

Paste this into Claude Code, Cursor, or your AI editor of choice at the root of your project:

```
Search this project for any imports from @airgap/beacon-sdk, @airgap/beacon-dapp,
or @airgap/beacon-types.

For each one found:
- If the imported symbol is NetworkType, SigningType, PermissionScope, Regions,
  RequestPermissionInput, RequestSignPayloadInput, or NodeDistributions, change
  the import to: import { ... } from '@taquito/beacon-wallet/types'
- If the imported symbol is BeaconEvent or DAppClientOptions, change the import
  to: import { ... } from '@taquito/beacon-wallet'
- If the imported symbol is something else, flag it for manual review.

Also check:
- package.json for direct dependencies or overrides referencing @airgap/beacon-*.
  These can be removed.
- Build configs (vite.config.ts, webpack.config.js, etc.) for references to
  @airgap/beacon-* in manualChunks, externals, or similar. Replace with the
  corresponding @ecadlabs/beacon-* package, but note there is no
  @ecadlabs/beacon-sdk meta-package — drop that entry or replace with the
  specific packages (@ecadlabs/beacon-dapp, @ecadlabs/beacon-ui,
  @ecadlabs/beacon-types).

Show me a summary of all changes before making them.
```

### Use `disconnect()` for logout, not `clearActiveAccount()`

Many Tezos dApps use `clearActiveAccount()` to implement "logout" or "disconnect" functionality. This is incorrect and contributes to the stale relay node problems described above.

`clearActiveAccount()` only removes the active account reference from `localStorage`. It does not clear other Beacon session state, including the cached relay node stored under the `beacon:matrix-selected-node` key. When relay nodes change or go offline, users with stale cached nodes get stuck in a state where wallet connections silently fail. The only recovery is manually clearing `localStorage`.

The correct method for logout is `disconnect()`, which calls `destroy()` under the hood and removes all `beacon:*` keys from `localStorage`. On next connection, the SDK selects a fresh, reachable node.
...
After calling `disconnect()`, the `BeaconWallet` instance is no longer usable. Create a new instance to reconnect:
...
#### Disconnect migration prompt

```
Search this project for any calls to clearActiveAccount() on a BeaconWallet
instance or its client property. Common patterns:

  await wallet.client.clearActiveAccount()
  await beaconWallet.client.clearActiveAccount()

For each one found, determine from context whether it's being used for:

1. Logout/disconnect (e.g., in a disconnect handler, logout button, cleanup
   function): Replace with await wallet.disconnect(). Note that after
   disconnect(), the BeaconWallet instance is no longer usable and a new
   instance must be created to reconnect.

2. Account switching within an active session: Leave as-is.
   clearActiveAccount() is correct when preserving the Beacon connection
   but changing the active account.

Flag any ambiguous cases for manual review. Show me a summary of all
changes before making them.
```

### Other changes

- **docs:** Updated all Beacon SDK import references in tutorial, signing, and wallet API docs (#3336)
- **docs:** Document `disconnect()` vs `clearActiveAccount()` lifecycle (#3329)
...
This release updates Taquito's Beacon SDK dependency to the latest [v4.8.0](https://github.com/airgap-it/beacon-sdk/releases/tag/v4.8.0) release from Papers AG. This update adds the new octez.io Matrix relay nodes operated by Trilitech to the server pool, alongside the existing Papers.ch relay nodes.
We understand from the Papers.ch team that their relay servers will continue to operate for the near term, so there is no immediate urgency for the ecosystem to rotate away from the existing server pool.
Regarding Octez.connect: we are currently evaluating it for a future Taquito release. Octez.connect is a fork of the Beacon SDK v5.0.0-beta.3 branch, which introduces changes to error handling, transport serialization, and protocol negotiation. We want to make sure we understand the impact of these changes on existing dApps and wallets before shipping them to Taquito users. We welcome any interoperability testing documentation or results from the Trilitech team and will factor them into our evaluation.

---

### Beacon Version Update
- @taquito/beacon-wallet
   - Updated Beacon version to v4.8.0 #3325 #3326
   - overrode Beacon SDK default matrix relay nodes with a curated list of Papers and Trilitech operated nodes (Synapse 1.148.0)
and emptied decommissioned regional nodes (Users can still pass their own matrixNodes to override Taquito's defaults)

### Website Improvements
- established dual licensing for website and source code #3322
- published Taquito architecture documentation #3323
- fixed factual errors across documentation #3327

### Internal Changes
- Updated dependencies #3324
...
### Beacon Version Update
`@taquito/beacon-wallet` - Updated Beacon version to v4.7.0 #3314

### Bug Fixes
`@taquito/https-utils` - Fixed fetch module overwrite and Electron detection #3310 #3302

### Internal Changes
`integration-tests` - Added new testnet `tezlinkshadownet` configuration #3315
`@taquito/saplings` - Added security comments for zero nonce in encrypted key decryption #3000

### Website Improvements
- Improved XSS protection in search results #3291
- Improved stability of ChatKit #3291
- Improved accessibility of carousels #3291
- Added sitemap and improved SEO #3313
- Improved performance and accessibility scores for Lighthouse/PageSpeed Insights #3309

# Taquito v24.0.1

## 🛠 Internal

- snyk dependencies updates #3243

## 🐞 Bug Fixes
- `@taquito/http-utils` - made node only imports conditional
- `@taquito/signer` - supplemented `bn.js` after removal of `elliptic` package
...
> This is official release of v24.0.0.
> There are only websites and internal changes since 24.0.0-RC.0

### New Features

- `@taquito/rpc` – Added support for the new RPC endpoint `getdestinationindex` #3261
- `@taquito/michel-codec` – Added support for new Michelson instructions: `INDEX_ADDRESS` and `GET_ADDRESS_INDEX` #3238
- `@taquito/local-forging` – Added support for Michelson instructions: `INDEX_ADDRESS` and `GET_ADDRESS_INDEX` #3238

### Improvements & Fixes
- `@taquito/rpc` – Updated multiple RPC types to align with **Protocol 24 (Tallinn)** schemas, including: VotingInfoResponse, OperationContentsAndResultMetadataExtended1, BakingRightsQueryArgumentsBase, AttestationRightsQueryArguments, OperationResultTransaction, ConstantsResponse, ConstantsResponseProto024, BlockMetadata #3244, #3245, #3261


### Dependencies

- Upgraded multiple Ledger-related dependencies via Snyk #3266
- Switched to native `fetch` where applicable #3266
- website dependencies update #3285 #3262

### Website

- kept page context when changing doc versions #3282
- added ecadinfra status page url to rpc_nodes.json #3283
- redirected old chatbot page to quick start with chatbot open #3284
- updated hardcoded version links to use default version #3289
- updated logo twitter to x #3288
- updated sapling terminology from "shielded pool" to "shielded set" #3281
...
The Sapling proving parameters are shipped as vendored package assets instead of being fetched during installation.
The output parameters live in `@taquito/sapling` and the spend parameters live in `@taquito/sapling-spend-params`, avoiding runtime downloads during installation while keeping publish artifacts manageable.
