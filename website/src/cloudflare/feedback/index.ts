import { DurableObjectNamespace, type DurableObjectState, type ExecutionContext } from '@cloudflare/workers-types';
...
async function handleVote(req: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
	...
	const csrfResult = await verifyCsrf(token, sid, env.CSRF_SECRET);
	if (!sid || !csrfResult) {
		return withCors(
			new Response(JSON.stringify({ error: 'CSRF failed', debug: { sid, token, csrfResult } }), {
				status: 403,
				headers: JSON_HEADERS
			}),
			origin
 ...
 */
async function signCsrf(sid: string, secretB64: string, timestamp?: number): Promise<string> {
	try {
		const key = await importHmacKey(secretB64);
		const ts = timestamp ?? Math.floor(Date.now() / 1000);
		const message = new TextEncoder().encode(`${sid}.${ts}`);
		const signature = await crypto.subtle.sign('HMAC', key, message);
		return `${b64url(new Uint8Array(signature))}.${ts}`;
	} catch (error) {
...
async function verifyCsrf(token: string, sid: string, secretB64: string): Promise<boolean> {
	try {
		...
		if (!Number.isFinite(timestamp) || now - timestamp > CSRF_EXPIRY_SECONDS) {
			return false;
		}

		// Re-sign with the SAME timestamp from the token to verify
		const expected = await signCsrf(sid, secretB64, timestamp);
		const expectedSig = expected.split('.')[0];
		const match = expectedSig === sigB64;
		return match;
	} catch (error) {
