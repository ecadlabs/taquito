...
async function example() {
  ...
  try {
    ...
    await op12.confirmation();

  console.log('Awaiting confirmation...');
  const contract12 = await op12.contract();
  console.log('Tzip12BigMapsOffChain Contract address',contract12.address)
  console.log('Gas Used', op12.consumedGas);
  console.log('Storage Paid', op12.storageDiff);
  console.log('Storage Size', op12.storageSize);
  console.log('Storage', await contract12.storage());
  console.log('Operation hash:', op.hash, 'Included in block level:', op12.includedInBlock);
    ...
    exec("cd ../integration-tests && sed -i 's/sandbox_known_tzip1216_contract/"+contract12.address+"/g' config.ts", (error, stdout, stderr) => {
      if (error) {
          console.log(`error: ${error.message}`);
          return;
      }
      ...
      console.log("Integration tests sandbox config updated with Tzip12BigMapsOffChain contract address: "+contract12.address);
    });
