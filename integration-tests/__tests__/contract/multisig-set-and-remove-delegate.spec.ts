...
const isRegisteredDelegate = async (Tezos: TezosToolkit, pkh: string) => {
  try {
    await Tezos.rpc.getDelegates(pkh);
    return true;
  } catch (error) {
    if (error instanceof HttpResponseError) {
      ...
      if (
        ...
        error.body.includes('delegate.not_registered')
      ) {
        return false;
      }
    }
...
CONFIGS().forEach(({ lib, rpc, setup, createAddress }) => {
  ...
  describe(`Generic Multisig set delegate: ${rpc}`, () => {
    ...
    test('test manager transfers set delegate scenarios', async () => {
      ...
      const pkh = await Tezos.signer.publicKeyHash();
      if (!(await isRegisteredDelegate(Tezos, pkh))) {
      ...
      const op2 = await Tezos.contract.originate({
        ...
        storage: {
          ...
          threshold: 2,
          keys: [
            await account1.signer.publicKey(),
            await account2.signer.publicKey(),
            await account3.signer.publicKey(),
          ],
        },
      });
      ...
      const op3 = await contract.methodsObject
        .main({
          payload: {
            counter: '0',
            action: {
      ...
      const packed2 = await Tezos.rpc.packData(
        pair(
          {
            data: {
              prim: 'Pair',
      ...
      const op4 = await contract.methodsObject
        .main({
          payload: {
            counter: '1',
            action: {
