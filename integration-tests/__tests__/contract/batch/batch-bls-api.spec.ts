...
CONFIGS().forEach(({ lib, rpc, setup, knownBaker, protocol, createAddress }) => {
  ...
  describe(`Test the Taquito batch api using: ${rpc}`, () => {
    ...
    // TODO: will need to optimize for bls keys fee of batch estimation besides reveal
    ...
    //   console.log('batchOp1', batchOp.status)
    //   expect(op.status).toEqual('applied')
    // })
