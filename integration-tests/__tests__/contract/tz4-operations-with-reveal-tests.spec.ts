...
CONFIGS().forEach(({ lib, rpc, setup, createAddress, knownBaker, knownTicketContract }) => {
  ...
  const createFundedTz4 = async () => {
    const tz4 = await createAddress(PrefixV2.BLS12_381SecretKey);
  ...
  const createFundedRevealedTz4 = async () => {
    const tz4 = await createFundedTz4();
  ...
  describe(`Test Tz4 consensus key updates through contract API using: ${rpc}`, () => {
    let Tz4: TezosToolkit;
  ...
  describe(`Test Tz4 companion key updates through contract API using: ${rpc}`, () => {
    let Tz4: TezosToolkit;
  ...
  describe(`Test Tz4 ticket and smart rollup operations through contract API using: ${rpc}`, () => {
    let Tz4: TezosToolkit;
    ...
    beforeAll(async () => {
      ...
      Tz4 = await createFundedRevealedTz4();
    });
