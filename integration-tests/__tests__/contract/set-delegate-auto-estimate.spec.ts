...
CONFIGS().forEach(({ lib, rpc, setup, knownBaker, networkName }) => {
  ...
  describe(`Test account delegation with estimation through contract api using: ${rpc}`, () => {
    ...
    notTezlinknet('Verify that an account can be delegated to known baker with automatic estimate', async () => {
    ...
    notTezlinknet('Verify that delegate can be withdrawn with automatic estimate', async () => {
      const pkh = await Tezos.signer.publicKeyHash();
      try {
        const op = await Tezos.contract.setDelegate({
          source: pkh,
