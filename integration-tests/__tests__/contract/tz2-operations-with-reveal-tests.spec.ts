...
CONFIGS().forEach(({ lib, rpc, setup, knownBaker, createAddress, knownTicketContract, networkName }) => {
  const Tezos = lib;
  ...
  describe(`Test tz2 account operations through contract API using: ${rpc}`, () => {
    ...
    notTezlinknet('verify that smart_rollup_originate fee and gas is sufficient', async () => {
    ...
    notTezlinknet('verify that smart_rollup_add_messages fee and gas is sufficient', async () => {
