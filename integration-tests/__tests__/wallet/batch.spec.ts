...
CONFIGS().forEach(({ lib, rpc, setup, knownContract, knownBaker, createAddress, networkName }) => {
  const Tezos = lib;
  ...
  test('Verify batch contract calls can specify amount, fee, gasLimit and storageLimit', async () => {
    ...
    if (isTezLink) {
      ...
      const batch = Tezos.wallet
        ...
        .withContractCall(
          contract.methodsObject.do(MANAGER_LAMBDA.transferToContract(knownContract, 1)),
