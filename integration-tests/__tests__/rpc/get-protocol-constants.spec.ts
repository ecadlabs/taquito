...
CONFIGS().forEach(({ lib, rpc, networkName }) => {
  const Tezos = lib;
  const weeklynet = networkName === 'WEEKLYNET' ? test : test.skip;
  const seoulnet = networkName === 'SEOULNET' ? test : test.skip;
  const shadownet = networkName === 'SHADOWNET' ? test : test.skip;
  const ghostnet = networkName === 'GHOSTNET' ? test : test.skip;
  const tezlinknet = networkName === 'TEZLINKNET' ? test : test.skip;
  const tallinnnet = networkName === 'TALLINNNET' ? test : test.skip;
  ...
  describe(`Fetch constants for testnet`, () => {
    ...
    shadownet(`should successfully fetch all constants for Shadownet
      using ${rpc}`, async () => {
      Tezos.setRpcProvider(rpc);
      const constants: ConstantsResponseProto023 = await Tezos.rpc.getConstants();
      expect(constants).toEqual({
    ...
    tezlinknet(`should successfully fetch all constants for Tezlinknet
      using ${rpc}`, async () => {
      Tezos.setRpcProvider(rpc);
      const constants: ConstantsResponseProto023 = await Tezos.rpc.getConstants();
      expect(constants).toEqual({
        ...
        liquidity_baking_toggle_ema_threshold: 1000000000,
        max_operations_time_to_live: 450,
        minimal_block_delay: new BigNumber(1),
        delay_increment_per_round: new BigNumber(4),
        consensus_committee_size: 7000,
        consensus_threshold_size: 4667,
        minimal_participation_ratio: {
    ...
    ghostnet(`should successfully fetch all constants for Ghostnet
      using ${rpc}`, async () => {
      Tezos.setRpcProvider(rpc);
      const constants: ConstantsResponseProto023 = await Tezos.rpc.getConstants();
      expect(constants).toEqual({
        ...
        minimal_stake: new BigNumber('6000000000'),
        minimal_frozen_stake: '600000000',
        vdf_difficulty: new BigNumber('600000000'),
        origination_size: 257,
        issuance_weights: {
          base_total_issued_per_minute: '80007812',
          baking_reward_fixed_portion_weight: 5120,
        ...
        max_operations_time_to_live: 450,
        minimal_block_delay: new BigNumber(4),
        delay_increment_per_round: new BigNumber(1),
        consensus_committee_size: 7000,
        consensus_threshold_size: 4667,
        minimal_participation_ratio: {
          numerator: 2,
        ...
        smart_rollup_arith_pvm_enable: false,
        smart_rollup_origination_size: 6314,
        smart_rollup_challenge_window_in_blocks: 302400,
        smart_rollup_stake_amount: '10000000000',
        smart_rollup_commitment_period_in_blocks: 225,
        smart_rollup_max_lookahead_in_blocks: 648000,
        smart_rollup_max_active_outbox_levels: 302400,
        smart_rollup_max_outbox_messages_per_level: 100,
        smart_rollup_number_of_sections_in_dissection: 32,
        smart_rollup_timeout_period_in_blocks: 151200,
        smart_rollup_max_number_of_cemented_commitments: 5,
        smart_rollup_max_number_of_parallel_games: 32,
        smart_rollup_reveal_activation_level: {
          raw_data: {
