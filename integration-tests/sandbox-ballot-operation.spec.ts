...
CONFIGS().forEach(async ({ lib, rpc, protocol, setup }) => {
  ...
  describe(`Test Proposal and Ballot operation in ${protocol.substring(0, 8)} with flextesa`, () => {
    ...
    flextesanet('Should be able to inject proposal operation in proposal period and check currentProposal in exploration', async (done) => {
      ...
      console.log(68, await Alice.rpc.getCurrentPeriod())
