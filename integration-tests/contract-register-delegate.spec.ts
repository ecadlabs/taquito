...
CONFIGS().forEach(({ lib, rpc, setup, protocol }) => {
  const Tezos = lib;
  _describe(`Test register delegate through contract api: ${rpc}`, () => {

    beforeEach(async () => {
      await setup(true)
    })
    _it('As a User I want to verify that I can register the current address as delegate using contract.registerDelegate', async () => {
      try {
