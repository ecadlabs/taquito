...
<<<<<<< HEAD
    ...
    kathmandunet('tests manager transfer scenarios with wallet APi contract', async (done) => {
      const op = await Tezos.wallet.originate({
        balance: "1",
        code: managerCode,
        init: { "string": await Tezos.signer.publicKeyHash() },
    ...
    mondaynet('tests manager transfer scenarios with wallet APi contract', async (done) => {
      const op = await Tezos.wallet.originate({
        balance: "1",
        code: managerCode,
        init: { "string": await Tezos.signer.publicKeyHash() },
