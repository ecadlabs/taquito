...
CONFIGS().forEach(({ lib, protocol, rpc, setup }) => {
  const Tezos = lib;
  ...
  describe(`On chain views using the contract API: ${rpc}`, () => {
    ...
    kathmandunet(`On K protocol as a user I want to originate a smart contract having top level views and simulate the views execution`, async (done) => {
    ...
    jakartanet(`On J protocol as a user I want to originate a smart contract having top level views and simulate the views execution`, async (done) => {
      // Contract origination
      const op = await Tezos.contract.originate({
        code: codeViewsTopLevel,
        storage: 2,
