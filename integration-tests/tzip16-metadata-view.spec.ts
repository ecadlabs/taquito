...
CONFIGS().forEach(({ lib, rpc, setup }) => {
  ...
  describe(`Execute views example from TZComet: ${rpc}`, () => {
    ...
    test('Deploy a contract with metadata having views that return bytes (example taken from TZComet) and call the views', async (done: () => void) => {
      ...
      expect(countRpc.get(`${rpc}/chains/main/blocks/head/context/constants`)).toEqual(2); //2
      expect(countRpc.get(`${rpc}/chains/main/blocks/head/metadata`)).toBeLessThanOrEqual(2);
       expect(
      ...
      expect(countRpc.get(`${rpc}/chains/main/chain_id`)).toBeLessThanOrEqual(4);
    ...
    test('Deploy a contract with metadata having a couple of views (example taken from TZComet) and call the views', async (done: () => void) => {
      ...
      expect(countRpc.get(`${rpc}/chains/main/blocks/head/context/constants`)).toEqual(1); //2
//      This value appears to alterate between 1 and 2
    ...
    expect(countRpc.get(`${rpc}/chains/main/blocks/head/header`)).toBeLessThanOrEqual(3);
      ...
      expect(countRpc.get(`${rpc}/chains/main/blocks/head/helpers/forge/operations`)).toEqual(2);//3
      expect(countRpc.get(`${rpc}/chains/main/chain_id`)).toBeLessThanOrEqual(4);
            expect(countRpc.get(`${rpc}/chains/main/blocks/head/helpers/scripts/run_operation`)).toEqual(
        1
      );
     ...
     // expect(countRpc.get(`${rpc}/chains/main/blocks/head`)).toEqual(19);
      ...
      //  ).toEqual(1);
       expect(
         countRpc.get(
           `${rpc}/chains/main/blocks/head/context/contracts/${contractAddress}/balance`
         )
       ).toBeLessThanOrEqual(3);
      expect(countRpc.get(`${rpc}/chains/main/blocks/head/helpers/scripts/run_code`)).toEqual(6);
      done();
    });
  });
});
