...
CONFIGS().forEach(({ lib, rpc, setup }) => {
  ...
  describe(`Originating contracts made with wallet api having metadata stored at HTTPS URL using: ${rpc}`, () => {
    ...
    test(
      'Deploy a contract made with wallet api having metadata stored at on IPFS',
      2,
      async (done: () => void) => {
    ...
    test(
      'Should fetch metadata of the wallet api made contract on IPFS',
      2,
      async (done: () => void) => {
        ...
        const signer = await Tezos.signer.publicKeyHash();
        expect(countRpc.get(`${rpc}/chains/main/blocks/head/metadata`)).toEqual(5);
        //expect(countRpc.get(`${rpc}/chains/main/blocks/head/context/contracts/tz2VayTveVoHv7hAgegbzHbksYCugtMibeB3/balance`)).toEqual(1);
        expect(countRpc.get(`${rpc}/chains/main/blocks/head/context/constants`)).toEqual(3);
        //expect(countRpc.get(`${rpc}/chains/main/blocks/head/context/contracts/tz2VayTveVoHv7hAgegbzHbksYCugtMibeB3/manager_key`)).toEqual(2);
        expect(countRpc.get(`${rpc}/chains/main/blocks/head/header`)).toEqual(4);
        //expect(countRpc.get(`${rpc}/chains/main/blocks/head/context/contracts/tz2VayTveVoHv7hAgegbzHbksYCugtMibeB3`)).toEqual(2);
        expect(countRpc.get(`${rpc}/chains/main/blocks/head/helpers/forge/operations`)).toEqual(2);
        expect(countRpc.get(`${rpc}/chains/main/chain_id`)).toEqual(1);
        expect(
          countRpc.get(`${rpc}/chains/main/blocks/head/helpers/scripts/run_operation`)
        ...
        //expect(countRpc.get(`${rpc}/chains/main/blocks/head/context/big_maps/138277/expru5X1yxJG6ezR2uHMotwMLNmSzQyh5t1vUnhjx4cS6Pv9qE1Sdo`)).toEqual(1);
        done();
      }
    );
  });
