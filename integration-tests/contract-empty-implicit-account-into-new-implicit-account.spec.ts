...
CONFIGS().forEach(({ lib, rpc, setup, createAddress, protocol }) => {
    ...
    describe(`Test emptying a revealed implicit account into a new implicit account using: ${rpc}`, () => {
        ...
        jakartanet('reveals the sender account, creates an unrevealed implicit account, empties the sender account into the created one', async (done) => {
        ...
        kathmandunet('reveals the sender account, creates an unrevealed implicit account, empties the sender account into the created one', async (done) => {
            const receiver = await createAddress();
            const receiver_pkh = await receiver.signer.publicKeyHash();
