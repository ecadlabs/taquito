  ...
  ({
    ...
    rpcList.forEach(async (rpc) => {
      ...
      const rpcClient = new RpcClientCache(new RpcClient(rpc));
        ...
        it('Verify the calling runView will exectue TZIP-4 views', async (done) => {
          let chainId: string;
        ...
        kathmandunetAndMondaynet('Verify that calling runSCriptView executes michelson view', async (done) => {
        ...
        kathmandunetAndMondaynet('Verify that rpcClient.getTxRollupInbox will access the inbox of a transaction rollup on kathmandunet', async (done) => {
          const inbox = await rpcClient.getTxRollupInbox(txRollupAddress, '0');
          expect(inbox).toBeDefined();
          done();
        });

        kathmandunetAndMondaynet('Verify that rpcClient.getTxRollupState will access the state of a rollup on kathmandunet', async (done) => {
           const state = await rpcClient.getTxRollupState(txRollupAddress);
           expect(state).toBeDefined();
           done();
         });
