...
CONFIGS().forEach(({ lib, setup, createAddress, rpc, protocol }) => {
  const Tezos = lib;
  ...
  describe(`Test estimate scenarios using: ${rpc}`, () => {
    ...
    mumbainet('should be able to estimate register global constant op', async (done) => {
      const estimate = await LowAmountTez.estimate.registerGlobalConstant({
        value: {
          prim: 'Pair',
