...
CONFIGS().forEach(({ rpc, knownContract }) => {
  ...
  _describe(`Test Taquito RPC: ${rpc}`, () => {
    _describe('Test getBlock', () => {
      _it('Verify that client.getBlock returns a block using default syntax', async () => {
        // defaults to /chains/main/blocks/head/
        const block = await client.getBlock();

        expect(block.protocol).toBeTruthy();
      });
      _it('Verify that client.getBlock({ block: \'head~2\' }) returns a block using head and tilde syntax', async () => {
      ...
      _it('Verify that client.getBlock({ block: `${block.hash}~1` }) returns a block using hash and tilde syntax', async () => {
        const block = await client.getBlock();
