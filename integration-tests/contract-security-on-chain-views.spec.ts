...
/**   
 * These "naive*" testcases assume that the on-chain view stack is not correctly separated from the caller's stack. 
 * Thus, this testcase assumes that there is only one stack, whereas the elements of the on-chain stack are on top. 
 * Naive - meaning: We just try it without thinking whether this test makes sense with regard to the used underlying architecture. 
 * We think of the underlying architecture (type system, stack separation, etc.) as a black box.
 * 
 * TC-V-001: On-chain view - add instruction
 * TC-V-002: On-chain view - dig instruction
 * TC-V-003: On-chain view - dup instruction
**/
          ...
          code: ` { parameter unit;
            ...
            view
              "rogue" unit nat
              {
                ...
                # Since add consumes two stack elements we have to push another nat (the result of the view) 
                # to the stack in order to restore a correct stack.
                PUSH nat 1;
              }
            }`,
          init: 'Unit'
