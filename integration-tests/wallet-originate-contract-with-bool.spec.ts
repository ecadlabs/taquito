...
CONFIGS().forEach(({ lib, rpc, setup }) => {
  const Tezos = lib;
  _describe(`Test contract origination with set bool prop on init and via call through wallet api using: ${rpc}`, () => {

    beforeEach(async () => {
      await setup()
    })
    _it('Verify wallet.originate for a contract with bool storage init to true and then sets to false', async () => {
