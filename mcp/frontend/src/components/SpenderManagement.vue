...
function useGeneratedKeypair(): void {
  if (keypairStore.generatedKeypair) {
    newSpenderAddress.value = keypairStore.generatedKeypair.address
  }
}
...
<template>
  <section class="card p-5 mb-5">
    ...
    <div class="card-subtle p-4 mb-4">
      <p class="text-sm font-medium text-text-primary mb-1">Generate New Spender Keypair</p>
      <p class="text-sm text-text-muted mb-3">
        Create a new keypair for the spender role. Save the secret key to use with the MCP server.
      </p>
