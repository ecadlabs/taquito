...
export interface ContractProvider extends StorageProvider {
  ...
  /**
   *
   * @description Unstake the given amount. If `amount` is larger than the currently staked amount, unstakes everything from the staking balance. Unstaked tez remains frozen for a set amount of cycles (the slashing period) after the operation. Once this period is over, the operation "finalize unstake" must be called for the funds to appear in the spendable balance. Will sign and inject an operation using the current context
   *
   * @returns An operation handle with the result from the rpc node
   *
  ...
  /**
   *
   * @description Transfer all the finalizable unstaked funds of the source to their spendable balance. Will sign and inject an operation using the current context
   *
   * @returns An operation handle with the result from the rpc node
   *
