...
describe('Configurations for the PollingSubscribeProvider', () => {
  ...
  it('Should be initialized with default config values', () => {
    ...
    expect(pollingSubscribeProvider.config.streamerPollingIntervalMilliseconds).toBeUndefined();
  ...
  it('Should set a streamerPollingIntervalMilliseconds property on instantiation', () => {
    const pollingSubscribeProvider = new PollingSubscribeProvider(mockContext, {
      streamerPollingIntervalMilliseconds: 2000,
    });
    expect(pollingSubscribeProvider.config.streamerPollingIntervalMilliseconds).toEqual(2000);
    expect(pollingSubscribeProvider.config.shouldObservableSubscriptionRetry).toBeFalsy();
    expect(pollingSubscribeProvider.config.observableSubscriptionRetryFunction.prototype).toEqual(
      retry().prototype
    );
  ...
  it('should set the streamerPollingIntervalMilliseconds property based on the minimal_block_delay constant', async (done) => {
    const pollingSubscribeProvider = new PollingSubscribeProvider(mockContext);
    ...
    expect(pollingSubscribeProvider.config.streamerPollingIntervalMilliseconds).toEqual(5000);
    expect(pollingSubscribeProvider.config.shouldObservableSubscriptionRetry).toBeFalsy();
    expect(pollingSubscribeProvider.config.observableSubscriptionRetryFunction.prototype).toEqual(
      retry().prototype
    );
  ...
  it('should set the streamerPollingIntervalMilliseconds property based on the time_between_blocks constant', async (done) => {
    const pollingSubscribeProvider = new PollingSubscribeProvider(mockContext);
    mockReadProvider.getProtocolConstants.mockResolvedValue({
      time_between_blocks: [new BigNumber('30'), new BigNumber('20')],
    });
    await pollingSubscribeProvider['getConfirmationPollingInterval']();
    expect(pollingSubscribeProvider.config.streamerPollingIntervalMilliseconds).toEqual(10000);
    expect(pollingSubscribeProvider.config.shouldObservableSubscriptionRetry).toBeFalsy();
    expect(pollingSubscribeProvider.config.observableSubscriptionRetryFunction.prototype).toEqual(
      retry().prototype
    );
  ...
  it('should use default polling interval if time_between_blocks is 0 (sandbox)', async (done) => {
    ...
    await pollingSubscribeProvider['getConfirmationPollingInterval']();
    expect(pollingSubscribeProvider.config.streamerPollingIntervalMilliseconds).toEqual(1000);
    expect(pollingSubscribeProvider.config.shouldObservableSubscriptionRetry).toBeFalsy();
    expect(pollingSubscribeProvider.config.observableSubscriptionRetryFunction.prototype).toEqual(
      retry().prototype
    );
