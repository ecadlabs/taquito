...
describe('ContractsLibrary tests', () => {
    ...
    it('ContractsLibrary is instantiable', () => {
        ...
        ).toBeInstanceOf(ContractsLibrary);
    });
    ...
    it('throw an InvalidScriptFormatError error if the script format is invalid', () => {
        ...
        try {
            ...
            expect(e.message).toEqual(`An invalid script property has been provided for ${contractAddress}. The script property can be retrieved from TezosToolkit.rpc.getScript(${contractAddress}). Invalid script: ${script}`);
            expect(e).toBeInstanceOf(Error);
        }
    });
