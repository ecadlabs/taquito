...
describe('Or token', () => {
  ...
  beforeEach(() => {
    ...
    tokenNoAnnots = createToken(
      { prim: 'or', args: [{ prim: 'int' }, { prim: 'string' }], annots: [] },
      0
    ) as OrToken;
    tokenComplex = createToken(
      {
        prim: 'or',
        args: [
          {
    ...
    tokenComplexNoAnnots = createToken(
      {
        prim: 'or',
        args: [
          {
            ...
            args: [
              {
                prim: 'or',
                args: [
                  { prim: 'pair', args: [{ prim: 'nat' }, { prim: 'int' }] },
                  { prim: 'string' },
                ],
              },
              { prim: 'mutez' },
  ...
  describe('EncodeObject', () => {
    it('Should throw unless input is an object with single key', () => {
      expect(() => tokenNoAnnots.EncodeObject(undefined)).toThrow(
        'EncodeObject expectes an object with a single key but got:'
      );
