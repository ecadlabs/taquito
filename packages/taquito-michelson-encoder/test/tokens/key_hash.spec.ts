...
describe('KeyHash token', () => {
  ...
  describe('compare', () => {
    it('Should compare key hashes properly', () => {
      ...
      expect(
        token.compare(
          'tz1VkT3u3Cpha5sZof3xJviHuRGugJDhpGET',
          'tz1b8nu4TNAFhn8wXpDjfvZeeT9P8DdqDFsW'
        )
      ).toEqual(-1);
      expect(
        token.compare(
          'tz1hyxDxcaZuNmoqYEQUqdatSLwfdBhX9x61',
          'tz1Xbi5kVw3iYvrcSegLWSDaRJkLSz31XE8f'
        )
      ).toEqual(1);
      expect(
        token.compare(
          'tz1PgiGQ2Bcg1FNBPdxxJdAJJ267FeQA78PF',
          'tz1geFsjAFibHEHypZyCmiGxkNpbyyyArtzs'
        )
      ).toEqual(-1);
      expect(
        token.compare(
          'tz2NjNtMdj6KeJNrnnctxPjMWgMZ6EozF4G8',
          'tz2Cv3bEm12m8M5he5rEHiGmeM5dz5Dt6Cn1'
        )
      ).toEqual(1);
      expect(
        token.compare(
          'tz2EPzvK7JPEz5cwpQ5Q5eFCaFA5FSbQzTrD',
          'tz2LjZSckDHtnTrM4i4zTReQ81cJS8Ufmfpd'
        )
      ).toEqual(-1);
      expect(
        token.compare(
          'tz2LRCiqdRDt82sY7z1m7T2Nxz35Kq9dsCF4',
          'tz2KAgSUucoJwiaVo8fJ5Z1Xu8JRnpcs8Q3C'
        )
      ).toEqual(1);
      expect(
        token.compare(
          'tz2SkvMjNH9pBaQG3N5vNBnsUykHVLxqmeQc',
          'tz2TPznBauRz2RSvWqSCvfL9aRjgURYikXxt'
        )
      ).toEqual(-1);
      expect(
        token.compare(
          'tz2Fs1PNZivSmaCRLDnDSAx9uA2a5LbyPiPs',
          'tz2ChnSybBcDTv7xxiKkH2WumCtv9UTS7hSU'
        )
      ).toEqual(1);
      expect(
        token.compare(
          'tz4S6JfUusG7pvmWyPyUBUrJRhHaZeFB32zc',
          'tz4QjLEb91k82QGhasmcgAzBoyiay5vLMgye'
        )
      ).toEqual(1);
      expect(
        token.compare(
          'tz4NFi9HLqcRaGPcaPJbonuzK37d7B76Kjgg',
          'tz4PEwe6H7cQG9Ce5nSHZcXvkXygJZTs3um7'
        )
      ).toEqual(-1);
      expect(
        token.compare(
          'tz4Hmxm1ULZcAwEfRw9bAX8czv5JCpKx3AE5',
          'tz4HBNXtBfqecLsAxEi9bHwvRMFqAcx3SCAz'
        )
      ).toEqual(1);
      expect(
        token.compare(
          'tz4XKRGpf8N2xDL9LbET8SzAMr1PawzqCmq1',
          'tz4HHJU6Yk89socL7mLemSaDbnZ5fvh8QWK8'
        )
      ).toEqual(1);
      expect(
        token.compare(
