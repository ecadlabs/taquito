...
describe('RpcClient test', () => {
  ...
  describe('getBlock', () => {
    ...
    it('should access the properties of the operation type tx_rollup_rejection, proto13', async (done) => {
      ...
      expect(content.message.batch).toEqual('');
      expect(content.message_position).toEqual('0');
      expect(content.message_path[0]).toEqual('txi1WZKF1fkUWfKbmaHbb5b8gn68rKSyUy4k7NnSVY4p79BKYz5RB');
      expect(content.message_result_hash).toEqual('txmr344vtdPzvWsfnoSd3mJ3MCFA5ehKLQs1pK9WGcX4FEACg1rVgC');
      expect(content.message_result_path[0]).toEqual('txM2eYt63gJ98tv3z4nj3aWPMzpjLnW9xpUdmz4ftMnbvNG34Y4wB');

      expect(content.previous_message_result).toBeDefined();
      expect(content.previous_message_result.context_hash).toEqual('CoVUv68XdJts8f6Ysaoxm4jnt4JKXfqx8WYVFnkj2UFfgKHJUrLs');
      ...
      expect((content.proof.before as { node: string }).node).toEqual('CoVUv68XdJts8f6Ysaoxm4jnt4JKXfqx8WYVFnkj2UFfgKHJUrLs');
      expect((content.proof.after as { node: string }).node).toEqual('CoUn3twa3TmvNby5VAGeN2jHvzbfpmJAXcyDHJuLLAuuLiaZZnzC');
      ...
      const otherEltsNode = ((content.proof.state[4] as { other_elts: OtherElts }).other_elts as { node: [string, { value: string } ][] }).node
      ...
      const otherEltsOtherElts = ((content.proof.state[5] as { other_elts: OtherElts }).other_elts as { other_elts: { value: any } }).other_elts
      expect(otherEltsOtherElts).toEqual({ value: "00000000" });

      expect(content.metadata.balance_updates).toBeDefined();

      expect(content.metadata.balance_updates![0].kind).toEqual('contract');
      expect(content.metadata.balance_updates![0].contract).toEqual('tz1MDU45gNc9Ko1Q9obcz6hQkKSMiQRib6GZ');
      ...
      expect(content.metadata.operation_result.balance_updates![0].contract!!).toEqual('tz1Lg9iLTS8Hk6kLfTN6rrrL9gYPfsTQ9z75');
      expect(content.metadata.operation_result.balance_updates![0].bond_id!.tx_rollup).toEqual('txr1V16e1hXyVKndP4aE8cujRfryoHTiHK9fG');
      expect(content.metadata.operation_result.balance_updates![0].change).toEqual('-10000000000');
      expect(content.metadata.operation_result.balance_updates![0].origin!).toEqual('block');

      expect(content.metadata.operation_result.balance_updates![1].kind).toEqual('burned');
      expect(content.metadata.operation_result.balance_updates![1].category!).toEqual('tx_rollup_rejection_punishments');
      ...
      expect(content.metadata.operation_result.balance_updates![2].category!).toEqual('tx_rollup_rejection_rewards');
      ...
      expect(content.metadata.operation_result.balance_updates![3].contract!).toEqual('tz1MDU45gNc9Ko1Q9obcz6hQkKSMiQRib6GZ');
