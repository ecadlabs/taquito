...
/**
 * Converts and validates Michelson sequences and expressions between plain Michelson and Micheline
 *
...
export class Parser {
  ...
  /**
   * Parses a Michelson sequence
   * @param src A string of raw michelson secquences `{parameter ...; storage int; code { DUP ; ...};}`
   */
  ...
  /**
   * Parse a Michelson expression
   *
   * @param src A Michelson expression such as `(Pair {Elt "0" 0} 0)`
   */
  ...
  /**
   * @description Takes Michelson in AST/JSON representation, strips away unneeded properties
   */
