...
const getWriteTransformFunc = (t: MichelsonType): WriteTransformFunc => {
  ...
  switch (t.prim) {
    case 'or':
      return (d: Expr) => {
        if (!isOrData(d)) {
          throw new MichelsonTypeError(t, `or expected: ${stringify(d)}`, d);
        }
        return [
          d,
          (function* () {
    ...
    case 'map':
      return (d: Expr) => {
        if (!Array.isArray(d)) {
          throw new MichelsonTypeError(t, `map expected: ${stringify(d)}`, d);
        }
        return [
          d,
            ...
            for (const _elt of d) {
              yield (elt: Expr) => {
                if (!('prim' in elt) || elt.prim !== 'Elt') {
                  throw new MichelsonTypeError(t, `map element expected: ${stringify(elt)}`, elt);
                }
                return [
                  elt,
                  (function* () {
