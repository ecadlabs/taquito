...
For this example we will use this contract:

https://github.com/murbard/smart-contracts/blob/master/multisig/michelson/generic.tz

> Due to a temporary limitation of Taquito we do not fully support raw michelson and need an external step to convert it to JSON. See https://github.com/ecadlabs/taquito/issues/100

## Convert the generic.tz contract to JSON

   We can use the tezos-cli to convert our michelson to JSON:

   ```sh
   tezos originate contract generic-multisig transferring 1 from account1 running "$(cat ./generic.tz)" --init "$(cat ./init.tz)" --dry-run --verbose-signing --burn-cap 2`
   ```
  ...
  When you will originate your contract with Taquito you will be able to provide the array which figure under the code property as your contract code namely
...
> Note: This requires a signer to be configured, ie: `Tezos.importKey("p2sk2obfVMEuPUnadAConLWk7Tf4Dt3n4svSgJwrgpamRqJXvaYcg1")`
  ...
  a. Using the storage encoder feature
    ...
    ```js
    // generic.json is referring to the JSON array above which we get from the tezos-cli output or other tools like the Ligo compiler
