...
Derivation paths are related to [Hierarchical Deterministic Wallet (HD wallet)](https://en.bitcoinwiki.org/wiki/Deterministic_wallet). `HD wallet` is a system allowing to derivate addresses from a mnemonic phrase combined with a derivation path. Changing one index of the path will allow accessing a different `account`. The number of addresses that can be accessed with `HD wallet` is unlimited. 
...
The path always begins with `44'/1729'` and we see some difference for the three other indexes across Tezos ecosystem. We can noticed that choosing any number for the three last indexes of the path (`account' / change / address_index`) will leads to different accounts. **But, to ensure consistency, it is important trying to follow the same convention regarding structure of the path and which index to increase to access the next address.**
...
For example, HDPathTemplate(0) (equivalent to `44'/1729'/0'/0'`) can be used to access the first address, HDPathTemplate(1) equivalent to `44'/1729'/1'/0'`) can be used to access the second address, HDPathTemplate(2) (equivalent to `44'/1729'/2'/0'`) can be used to access the third address... *To insure flexibility across all users, this template is not imposed by Taquito*.

We can see other implementation using `44'/1729'/0'/0'/0'`, which adheres to BIP44, where next address is accessed by incrementing `account` or `address_index`.
...
According to BIP44, "Software should prevent a creation of an account if a previous account does not have a transaction history (meaning none of its addresses have been used before)." When building app using the `LedgerSigner`, attention should be paid to not let users accessed account with path structure that does not follow any convention. Otherwise, users could have difficulties using their account with other wallets that are not compatible with path used. As stated before, an infinity of address can be accessed with HD wallet. According to BIP44, wallets should follow an `Account discovery` algorithm meaning that it is strongly possible that an account created with an unconventional path won't be found by the wallet. We can think about how hard it would be for a user who had created an account with a no common path and forgot it to find it back. 
...
Having your Ledger device connected to your computer and the `Tezos Wallet App` opened, you can run the following code example. It will scan your Ledger from path `44'/1729'/0'/0'` to `44'/1729'/9'/0'` to get public key hashes and the balance for accounts that are revealed. Confirmations will be asked on your Ledger to send the public keys.
